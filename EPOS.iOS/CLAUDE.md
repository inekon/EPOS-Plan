...
| `App.cs`, `HauptSeite.cs` | ein Fenster, eine Seite, eine `BlazorWebView` mit `EPOS.UI.Seiten.AppWurzel` — seit iU9‑W16c ist das die **gemeinsame Wurzel beider Plattformen** (Entscheid E‑1: eine Wurzel, zwei Schalen). Die Schale ist ein `RenderFragment` (`Kopfleiste`): Unter Windows steht dort das `Menueband` mit seinen 57 Punkten, **auf iOS bleibt sie leer** — iL5 sagt „kein MDI, keine modalen Ketten", und eine Menüleiste wäre auf Touch unbedienbar. Ohne Angabe macht die Wurzel mit `PROJEKTLISTE` auf, wie bisher |
