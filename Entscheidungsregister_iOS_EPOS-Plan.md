...
| **iF27** | **`bundle_green` ist tot** — fährt iOS dieselbe SQLite 3.53.3 wie Windows (`bundle_e_sqlite3`, statisch gelinkt)? | **ja.** Die Fassung 2.1.12 von `bundle_green` existiert nicht (letzte 2.1.11, in 3.0 entfallen), und die System-SQLite des Geräts wäre für die **114 STRICT-Tabellen** nicht steuerbar. Auf iOS liefert `bundle_e_sqlite3` `provider.internal` und eine statisch gelinkte `e_sqlite3.a` — dieselbe Fassung auf allen vier Läufern | iU10 | **umgesetzt und bestätigt** — CI-Lauf 33748736894 (03.09.2026): `SQLite 3.53.3`, `STRICT=114`, Projekt 1030 byte-gleich zur Windows-Basis | | |
