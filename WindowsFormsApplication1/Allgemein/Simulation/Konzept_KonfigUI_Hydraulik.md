...
| K-3 | ~~Bivalenz-Umschaltung ALTERNATIV schaltet heute je Stunde nach Leistungsunterdeckung, nicht nach Bivalenz-Temperatur (`Abschaltpunkt` wirkt nur TEILPARALLEL)~~ | **entschieden und umgesetzt am 15.08.2026** (Nutzerfreigabe als Ergebnisänderung): Umschaltung an der Bivalenztemperatur `Abschaltpunkt` in **beiden** Rechenwegen — `T_außen < Abschaltpunkt` → WP aus, sonst WP an; der Spaltenwert gilt immer wörtlich, auch 0 °C. Umsetzung, Datenbefund und Verifikation in [`K3_BivalenzTemperatur_Protokoll.md`](K3_BivalenzTemperatur_Protokoll.md); Regressionsbasis `Referenzlaeufe/2026-08-15_B3/`. Offener Folgeschritt: Sichtbarkeit des Eingabefelds in `Wizard_WPItem` auch für ALTERNATIV |
