> Archivierte Fassung der `EPOS.Kern/CLAUDE.md` vom 12.09.2026, vor der Kürzung auf den gültigen Stand. Die relativen Verweise sind auf diesen Ablageort umgeschrieben; Zählungen, Daten und Kürzel beschreiben den Stand von damals.
...
| `Allgemein/KI/` (26) | seit iU5-U2 das, was der Assistent **weiß**, und seit Auftrag #201 auch das, was er **tut**: `HilfeWissen` (`WissensAbschnitt`), `WikiWissen`, `SemantikIndex`, `SemantikModell` (ONNX), `KiSchreibschutz`, `KiSicherungspunkt`, `KiEinwilligung`, `KiTextlieferant`, `Aktionen/KiAktionsTexte`, `Dialoge/KiDialoge`, `Dialoge/KiDialogTexte`. **Seit Auftrag #199 (Stufe S1 „Der Hilfe-Assistent im Dialog") drei Dateien mehr:** `KiAufrufkontext` (Bereich, Dialogname, Hilfeschlüssel, Frage, Kennung — das eine Argument von `Dienste.Navigation.OeffneMaske(Masken.KiAssistent, …)`), `KiVerfuegbarkeit` (die EINE Auskunft „darf diese Installation den Assistenten anbieten?", ohne Netz-, Schlüssel- und Einwilligungsfrage — Anwenderentscheid KI‑D‑Q1) und `KiMeldungskennung` (die 15 Kennungen der erklärbaren Meldungen samt der vorbelegten Frage `KI_FRAGE_<Kennung>`). `KiChatKontext` trägt dazu die Tabelle Hilfeschlüssel-Präfix → Bereich und den gemeldeten Aufruf, `HilfeWissen` das **Aktionswissen** je Kennung. **Seit Auftrag #200 (Stufe S2) eine Datei mehr:** `KiMaskenbruecke` — der Weg vom offenen Razor-Dialog zum Assistenten. Der Dialog meldet je Katalogfeld einen Getter (und, für S3, einen Setzer) an; `Lesen` liefert Anzeigename, Art, Einheit, Rohwert UND Text, `Dialogdaten` den wörtlichen Feldblock, den Anfrage und Vorschau gleichermaßen mitnehmen, und `Vermerken` die eine Protokollzeile je Übertragung (`KiProtokoll.Zeile`, angehängt über die Senke der Plattformhülle). Ein Eintrag je Maske, die jüngste Anmeldung gilt, `Abmelden` trägt die Marke ihrer Anmeldung. **Der Dialogkatalog nennt seither Eigenschaften statt Controls** (`HeizkesselKatalogDaten.Ptherm`, Regel `KiEigenschaftspfad` in KiKern) und führt mit `StromspeicherAuslegung` eine fünfte Maske; `KiEinwilligung` trägt neben der allgemeinen die **zweite Stufe „Dialogdaten"** (KI‑D‑Q2: eigener Merker, eigene Fassung, eigenes Datum, eigener Rückweg — ohne eingehängten Haken kein Weg zu ihr), und `KiChatService` nimmt den Feldblock als EINEN optionalen Parameter entgegen, ohne Schalter oder Einwilligung nachzuprüfen. **Seit Auftrag #201 (Stufe S3) liegt das AKTIONSREGISTER hier** — die neun Dateien `Aktionen/KiAktionen` (trägt `KiHilfe`), `-Dialog`, `-Energie`, `-Lastgang`, `-Projekt`, `-Rechnen` (neu), `-Schreiben`, `-Sitzung`, `-Uebernahme`, `-Wirtschaft` und der Ausführer `KiAusfuehrung` (`IKiAusfuehrung`, **Instanz statt statisch**: Einläufigkeit, Laufmarke, Sitzungsgedächtnis und Register gehören einem Ausführer, und ein Prüfling legt sich einen frischen an). **29 Aktionen**, davon fünf neu: `dialog_oeffnen` (Stufe 1, über die Datentabelle `Dialoge/KiMaskenziele` — ein Wächter verlangt zu jedem Katalogeintrag ein Ziel), `dialog_speichern` (Stufe 2, die EINZIGE Dialogaktion mit Sicherungspunkt — KI‑D‑Q4) und die drei Rechenaktionen `simulation_rechnen`, `peak_ziel_bestimmen`, `flotte_bewerten` (Stufe 3, über `KiAktion.AusfuehrenLang` mit `KiKern.KiLaufumgebung` = Fortschritt + Abbruchmarke). Die Brücke trägt dazu die **Haken** der Maske (`KiMaskenhaken`: Auffrischen, Prüfen, Schreibschutz, Speichern, `Rechenweg(aktionsname)`) und `Dialoge/KiFeldwandler` die eine Umsetzung Text → Wert (nach `ZahlText.Parsen`, dazu Ja/Nein); `Dialoge/KiFeldhilfe` ist die stille Naht zum Hilfekatalog der Plattform. Was er **bedient**, bleibt bei der Oberfläche: Die Windows-Hülle hält nur noch `Form.ActiveForm.Modal`, den Hilfekatalog und den Fadenwechsel des Chatfensters. **Mit Auftrag #214 kommen Fortschritt und Abbruch wirklich an** (der Restpunkt aus #201): Eine Aktion mit `AusfuehrenLang` läuft in `KiAusfuehrung.ImHintergrund` (`Task.Run`) statt über `AufOberflaeche` — auf dem Bedienfaden wären Balken und Abbruchknopf des Chats eine Zusage, die niemand einlösen kann —, `simulation_rechnen` reicht einen `Phasenmelder` und `u.Abbruch` in `SimulationRunner.Simuliere`/`SimuliereUndSpeichere` (dort seither wahlfrei, der Referenzlauf bleibt byte-gleich), und die zwei Ansichtsrechnungen hängen `u.Abbruch` an denselben Abbruchweg wie der Knopf der Ansicht. Die **19 kurzen Aktionen bleiben auf dem Oberflächenfaden**; Wache samt Gegenprobe: `EPOS.Kern.Tests/KiRegisterS3Tests` |
...
| `KI/HilfeKontext`, `KI/KiAusfuehrungWindows` | `HilfeKontext` ermittelt das aktive Fenster; `KiAusfuehrungWindows` (Auftrag #201, 120 Z.) hält die eine Instanz der Kern-Ausführung und die zwei Haken, die nur Windows beantworten kann: `Form.ActiveForm.Modal` und den Hilfetext eines Katalogfeldes über `WikiHelpCatalog`. **`KiDialogZugriff` (567 Z., Controlsuche) und `KiAusfuehrungAdapter` sind mit #201 GELÖSCHT** — der Weg über `Application.OpenForms`/`FindControlRecursive` war seit iU9 tot, die Masken sind Razor-Komponenten. `KiAufrufKnopf` ist mit iU9‑W14a gefallen und mit iU9‑W15b.5 durch den Baustein `KiKnopf` ersetzt |
| ~~`KI/Aktionen/*` und `KiAusfuehrer`~~ | **mit Auftrag #201 in den Kern gezogen** (neun Dateien per `git mv`, der Ausführer als `KiAusfuehrung`). Die Begründung (nur im App-Projekt darf DB-Code stehen, Fachkonzept 3.7) war mit iU4 hinfällig: Die Controller liegen längst in `EPOS.Kern`, das Register liegt damit weiterhin in DERSELBEN Assembly wie sie. Was der Umzug bringt: **iOS und die Razor-Dialoge bekommen dasselbe Register** — vorher stand dort `KeineAusfuehrung` mit einem leeren. Die zwei OleDb-Stellen der Aktionsdateien sind dabei auf `SqliteException` umgestellt (Muster `PeakShavingCtrl`), der `HelpEntry`-Zugriff auf die Naht `KiFeldhilfe`. **`KiChatService` steht seit iU9‑W15b.0a HIER im Kern** (Befund W15b‑B1); die Naht zur Ausführungsschicht ist `IKiAusfuehrung`/`KiAusfuehrungsweg` |
...
[`Konzept_Einheiten_EPOS-Plan.md`](../../../../Dokumentation/aktuell/Konzept_Einheiten_EPOS-Plan.md).
