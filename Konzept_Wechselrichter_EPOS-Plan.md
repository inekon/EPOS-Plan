...
| Menüpunkt | `Menuetabelle.cs` (Administration → Energiesysteme, Administration → Daten & Import; seit **W16c‑E‑7** vom 07.09.2026 je unter einem Zwischenknoten „Photovoltaik") | **nicht dabei** |
...
Menüpunkt: **Administration → Daten & Import → Photovoltaik → „Wechselrichter (CEC, OND)…"**,
neben `MenuItem_PV_Import_CEC` (`EPOS.UI/Bausteine/Menuetabelle.cs`). (Rev. 3 nannte den Weg
noch ohne den Zwischenknoten „Photovoltaik"; den legt der Anwenderentscheid **W16c‑E‑7** vom
07.09.2026 über die zwei PV-Importpunkte — Ziel, Argument und Kennung des Punktes sind
unverändert, es wandert nur seine Lage im Baum und seine Beschriftung: Der Modulimport heißt
seither „PV Module (CEC, PAN)…" statt „Import Photovoltaik CEC/Pan".)
...
Menüpunkt: **Administration → Energiesysteme → Photovoltaik → „Wechselrichter"**,
unmittelbar nach „PV Module". (Rev. 1 nannte hier ein Untermenü „Photovoltaik → Bearbeiten";
das ist mit dem Anwenderentscheid **W16c‑E‑6** vom 06.09.2026 aufgelöst — `MenuItem_PV` trägt
sein Ziel seither selbst. Der Zwischenknoten **„Photovoltaik"** über beiden ist etwas
anderes und kommt mit **W16c‑E‑7** vom 07.09.2026: Er klappt nur auf und fasst die zwei
Kataloge EINER Anlage zusammen, weshalb „Photovoltaik Module" darunter „PV Module" heißt.)
Seitenschlüssel `WechselrichterAdmin`.
...
* **Neigung und Azimut je Strang stehen in Klammern, solange sie leer sind** — die Klammer zeigt
  ...
  leert, erbt wieder.
* **Die Spaltenfolge stellt die Zahlen nach vorn** (Befund **W6‑B‑4**, 07.09.2026):
  Wahl · Rang · Bezeichner · **Gerät · MPPT · Module in Reihe · Stränge parallel · Neigung ·
  Azimut** · Modul · Wechselrichter. Der Grund ist der Unterschied der Naturen: Ein Zahlenfeld
  hat eine feste, kleine Breite; eine Klappliste ist so breit wie ihr längster Eintrag und lässt
  sich nur **deckeln** (`max-width: 11rem`) und **abschneiden** (`text-overflow: ellipsis`, der
  volle Name im Werkzeugtipp). Was gedeckelt werden muss, gehört ans Ende — dort kostet das
  Abschneiden am wenigsten, und keine Zahl kann davon nach rechts geschoben werden. Gemessen bei
  1 100 px Fensterbreite (1 048 px im Dialog): vorher **1 974 px** Tabellenbreite gegen
  1 046 px sichtbar (928 px lagen daneben), nachher **1 046 px,
  `scrollWidth == clientWidth`** — die Tabelle rollt nicht mehr.
* **Ein NEU angelegter Strang ist vorbelegt** (W6‑B‑4): der erste mit der „Anzahl Module" der
  Anlage, jeder weitere mit den noch nicht zugeordneten Modulen (mindestens 1), dazu ein Strang
  parallel, Gerät 1 und der nächste freie MPP-Tracker des gewählten Geräts, sofern der Katalog
  dessen `Anzahl_Mppt` führt. Gerechnet wird das im **Kern**
  (`Strangvorbelegung.FuerNeuenStrang`) — eine Wahrheit für Windows und iOS. **Die Vorbelegung
  ersetzt keine Prüfung:** P1 bis P8 laufen danach wie bisher und sagen, ob die Reihe ins
  Spannungsfenster passt; sie sorgt nur dafür, dass P8 (Modulsumme gegen die Anlage) im
  Regelfall gleich still ist, statt den Planer mit „0 Module in Reihe" zu begrüssen.
* **Fehlt ein Modulwert, nennt die Ampel IHN — und den Pflegeweg** (W6‑B‑4). Statt
  „Leerlaufspannung **oder** beta_OC des Moduls" steht dort jetzt genau der Wert, den der
  Prüfstand vermisst (`U_OC`, `U_MPP`, `I_SC`, `alpha_SC`, `beta_OC`), jeder höchstens einmal je
  Meldung, und dahinter ein Satz: „Pflege: Administration → Energiesysteme → Photovoltaik →
  PV Module → Bearbeiten (Felder alpha_SC, beta_OC, T_NOCT) oder Neuimport aus
  „CEC Modules.csv"" (`PVS_PFLEGEWEG`, beide Sprachen). Der Satz steht nur da, wenn wirklich ein
  MODULwert fehlt — ein Hinweis, der immer dasteht, wird nicht gelesen.
...
| S1.4 | Dritte Ausprägung von `ModulKatalogProfil`/`ModulKatalogDialog` (25 Felder, drei Gruppen, Herstellerfilter), Menüpunkt, `Masken`/`Seitenschluessel`, `WinFormsNavigation`, `WechselrichterAdminHuelle`, Parameterübersicht (achte `Anlagenart`) | **umgesetzt** (`d7d25f3`); der Menüweg heißt seit **W16c‑E‑7** (07.09.2026) Administration → Energiesysteme → **Photovoltaik** → „Wechselrichter" (Kapitel 6, Kapitel 12) |
| S1.5 | `CecWechselrichterDienst`, `CecWechselrichter`, `WechselrichterKennlinie` (Sandia→Stützstellen), `WechselrichterImportDialog` + Hülle, Menüpunkt, Importprobe `cec_wechselrichter_21.csv` | **umgesetzt** (`119537f`); Abweichung zur „zweiten Ausprägung" in 5.5 benannt, offener Punkt W6‑O‑1. Der Menüweg heißt seit **W16c‑E‑7** Administration → Daten & Import → **Photovoltaik** → „Wechselrichter (CEC, OND)…" (5.2/5.5, Kapitel 12) |
...
7. ~~**Die Katalogpflege nicht vergessen.**~~ — **erledigt am 07.09.2026** (Befund **W6‑B‑5**,
   Entscheide Q1–Q3, Migrationsschritt **69**). Die Prüfungen P1–P4 hängen an `U_Leerlauf`,
   `U_Mpp`, `I_Kurzschluss`, `alpha_SC` und `beta_OC` der **Module**. Diese Werte waren im
   Bestand nachweislich verdorben (Paket-A-Befund A1; Handlauf unter `sql/pv_katalog/`), und
   **ohne Katalogpflege leuchtete die Ampel grau, nicht grün**. Der Schemaschritt 69
   repariert sie jetzt beim Update aus der CEC-Liste — Katalog **und** Projektkopien; was
   keinen Treffer hat, wird ehrlich leer statt geraten. Für Ablytek und Jinkosolar meldet die
   Ampel damit kein „fehlt" mehr, wo die Liste die Werte hergibt.
   **Nachtrag 06.09.2026:** W6‑O‑7 ist umgesetzt — Projekt 1045 mit Ost/West-Strängen hängt den Strangweg ins Netz,
   ...
   seit W6‑B‑5 `2026-09-07_R6_PvKoeffizienten`.
...
**Stand 07.09.2026 (zweiter Nachtrag):** Aus derselben Windows-Abnahme sind zwei Befunde dazugekommen —
**W6‑B‑4** (vier Beobachtungen an EINEM Bildschirmfoto des PV-Dialogs, alle vier behoben) und **W6‑B‑5** (die
**verdorbenen PV-Modulkoeffizienten** aus Paket‑A‑Befund A1, ohne die die Ampel P1–P4 grau bleibt; Kapitel 10,
Punkt 7 — behoben durch Schemaschritt 69). Beide stehen als erste Zeilen der Tabelle unten.
...
Aus dem Kern-CI-Lauf 216 kam als Nachlauf zu W6‑B‑2 der Punkt **W6‑B‑2‑O‑1** dazu (der
...
| **W6‑B‑4** | **Die Strangtabelle rollte waagerecht, ein neuer Strang begann mit 0 Modulen, die Meldung nannte nicht WAS fehlt und WO man es pflegt, und die Modul-Klappliste zeigte einen Namen, den die Zeile nicht trug.** Windows-Abnahme 07.09.2026, wörtlich: „Anzahl Module fehlt? Welche Werte?" Das Bildschirmfoto zeigt die Spalten bis „Gerät", einen waagerechten Rollbalken und darunter „Strang 1s1: 0 Module in Reihe, 1 parallel · Werte fehlen: Module in Reihe, Leerlaufspannung oder beta_OC des Moduls, MPP-Spannung oder beta_OC des Moduls". | **URSACHEN BELEGT und BEHOBEN, vier Punkte.** **(A) Die Zahlenspalten lagen ausserhalb des sichtbaren Bereichs.** Gemessen mit einer Playwright-Probe (Blazor-Wirt im Laufordner, `prerender: false` — derselbe Zeichenweg wie die WebView2, Chromium, Fensterbreite 1 100 px, im Dialog davon 1 048 px): Die Tabelle war **1 974 px** breit, `scrollWidth 1 974` gegen `clientWidth 1 046`, und sechs der elf Spalten standen vollständig rechts daneben. Drei Ursachen: die zwei `&lt;select&gt;` waren so breit wie ihr längster Eintrag („SMA America: SB30-1SP-US-40 {240V}" = 302 px), die sechs Zahlenfelder so breit wie ein `&lt;input&gt;` von Haus aus (je 196 px), und die Spaltenköpfe brachen wegen `.epos-raster th { white-space: nowrap }` nicht um. Behoben durch die Spaltenfolge aus Kapitel 7 (alles Schmale zuerst, die zwei elastischen Listen zuletzt) und drei Regeln im Hausblatt: `.epos-strangtabelle-liste .epos-eingabe` (Deckel 11 rem, `text-overflow: ellipsis`, voller Name im `title`), `.epos-strangtabelle th.epos-strangtabelle-zahl, …td…` (`width: 1%`, `white-space: normal` — **der Selektor nennt `th`/`td`**, weil die Hausregel (0,1,1) wiegt und eine blosse Klasse (0,1,0) schlüge) und `.epos-strangtabelle-text .epos-eingabe` (8,5 rem). **Nachher: 1 046 px, `scrollWidth == clientWidth`, die Seite rollt nicht.** **(B) Ein neuer Strang begann mit „0 Module in Reihe".** `StrangAnlegen()` legte `new StrangZeile { Rang = … }` an; `ModuleReihe`, `StraengeParallel`, `Mppt` und `Geraetenummer` sind `int?` und blieben `null`. Die Anlage KENNT ihre Modulzahl. Neu: `EPOS.Kern/Allgemein/Import/Strangvorbelegung.cs` — erster Strang = Modulzahl der Anlage, jeder weitere = der Rest (mindestens 1), parallel 1, Gerät 1, MPPT = nächster freier Tracker des Geräts (`WechselrichterStammCtrl.TrackerZahl`, `null` = unbekannt = 1). **(C) Die Meldung nannte ein Paar mit „oder".** `SpannungReihe` gibt `null`, wenn `U_STC` ODER `beta_OC` fehlt — der Code WUSSTE, welcher der beiden, und sagte es nicht; ausserdem gibt sie auch dann `null`, wenn allein `reihe = 0` ist, und klagte deshalb über zwei Modulwerte, die gepflegt waren (genau das im Bildschirmfoto). Neu: `Fehlliste` mit `SpannungsWerteFehlen`/`StromWerteFehlen` — je Wert eine Meldung, jeder höchstens einmal, und dahinter EIN Satz mit dem Pflegeweg (`PVS_PFLEGEWEG`), aber nur wenn wirklich ein Modulwert fehlt. Fünf neue Wertetexte (`PVS_FEHLT_UOC_WERT`, `…UMPP_WERT`, `…ISC_WERT`, `PVS_FEHLT_ALPHA_SC`, `PVS_FEHLT_BETA_OC`), beide Sprachen. Die Parameterübersicht des Moduls (W6‑E‑1) führte die drei Koeffizienten bereits, zeigte aber nur einen Halbgeviertstrich; sie schreibt seither „**– nicht gepflegt**" (`PVS_NICHT_GEPFLEGT`, `PhotovoltaikStammCtrl.ParameterNichtGepflegt`). **(D) Die Klappliste zeigte einen Namen, den die Zeile nicht trug.** Der leere Eintrag „(Modul der Anlage)" war da und war auch gewählt — aber **nur über das Attribut `value` des `&lt;select&gt;`**, und ein `&lt;select&gt;` hat im DOM gar kein `value`-Attribut: Der Blazor-Zeichner merkt sich den Wert beim Einhängen und setzt danach `element.value` nach (`_blazorDeferredValue`) — **genau einmal, beim Erzeugen des Elements**. Werden die `&lt;option&gt;` später ausgetauscht, patcht Blazor sie an Ort und Stelle, der Browser behält seinen `selectedIndex`, und in der Zeile steht der Name, der zufällig an diesem Platz gelandet ist. **Gemessen und reproduziert:** Ein Strang mit „SMA America: SB30-1SP-US-40 {240V}" zeigte nach einem Wechsel des Herstellerfilters „ABB: PVI-3.0-OUTD-S-US-A {208V}" — ein Gerät, das er nie hatte; im Markup stand **kein einziges `selected`**. Behoben im Standard `Auswahlfeld`, nicht im Wirt: jede `&lt;option&gt;` trägt seither `selected` UND ein `@key` auf ihrer Id — das `selected` legt die Wahl dorthin, wo der Browser sie ohnehin liest, das `@key` gibt jeder Option eine Identität, damit ein Austausch der Liste sie nicht unter einem fremden Eintrag stehen lässt. Nach dem Fix bleibt die Zeile über beide Filterwechsel bei ihrem Gerät. **bunit sieht das nicht** (Lehre W6‑B‑1): Sein Htmlizer schreibt `selected` aus dem `value` heraus — der Nachweis ist die Playwright-Probe, im Repository stehen die Regeln als `StilblattTests`-Fälle und die Spaltenfolge, die Vorbelegung und die Ampelmeldungen als bunit- und Kernfälle. **Nachweise:** 12 neue bunit-Fälle in `PvStraengeFelderTests` (Spaltenfolge, Spaltenklassen, Vorbelegung erster/zweiter Strang, drei Trackerfälle, `selected` am Rückfall und am eigenen Modultyp, Ampel gegen das Anlagenmodul), 2 in `StilblattTests`, **13 Kernfälle** in `StrangvorbelegungTests` (bis hin zu „P8 bleibt grün") und **7** in `StrangPlausibilitaetTests` (je Wert einer, Pflegeweg genau einmal, Gegenprobe ohne fehlenden Wert, Englisch), 1 in `PvModulparameterTests`; Referenzlauf 1030/1007/1017/1045 byte-gleich gegen R5 |
...
| **W6‑B‑2‑O‑1** | **Der Prüffall zu W6‑B‑2 war flatterhaft.** Kern-CI-Lauf **216** (`pull_request`, Head `833ff69`) rot mit genau einem Fall von 3 202: `ModulImportDialogTests.Der_Herstellerfilter_zeigt_nur_noch_die_Zeilen_des_Herstellers`, „Expected 5, Actual 155". Der Push-Lauf **215** auf DEMSELBEN Commit war grün. | **URSACHE BELEGT und BEHOBEN — ein Wettlauf IM TEST, kein Fehler des Dialogs.** bunits SYNCHRONE Ereignisse (`Click()`, `Change()`, `Input()`) geben das Ereignis nur beim Zeichner ab und warten nicht auf den Behandler; nur die `…Async`-Fassungen tragen laut bunit-Dokumentation „a task that completes when the event handler is done". Der Zeichner arbeitet das Ereignis auf dem Prüffaden ab, **solange seine Warteschlange frei ist** — liegt dort schon ein Werkstück, wird es eingereiht, und die nächste Zeile des Falls liest den Stand VOR dem Ereignis (hier: 155, die ungefilterte Zeilenzahl). Das Werkstück legt der Ladegang selbst hin: 155 Zeilen liegen über `VIRTUALISIEREN_AB` (120), das Raster schaltet auf `Virtualize`, und QuickGrid meldet sich mit `OnAfterRenderAsync` samt JS-Aufruf zurück — genau der Schalterwechsel, um den W6‑B‑2 selbst ging. **Gemessen** mit dem wörtlichen Prüfstand: ohne Fremdlast 0 von 600 rot, unter Rechenlast **68 von 400**, mit einer einmal nachgebenden Quelle (`Task.Yield` — die echte ruft das Netz) **58 von 60**; mit dem neuen Muster jeweils **0**. **Behebung nur am Test** (Muster W16b‑O‑2, `c3b1513`): Helfer `Geladen`/`Gefiltert`/`Gemeldet`/`Ueberlagert` in `ModulImportDialogTests` und `Einlesen(cut, n)`/`Gezeichnet`/`Markiert`/`Gemeldet` in `KatalogImportDialogTests` warten auf den GEZEICHNETEN Stand (Statuszeile mit der Trefferzahl, Auswahlzeile „… von n Einträgen geladen.", wieder bedienbare Quellenknöpfe); der Fehlerweg wartet auf das gezeichnete Warnbanner. Nachweise: der Fall 30/30 grün in `de` und `en_US.UTF-8`, beide Importklassen je 30/30 unter Rechenlast, gesamte `EPOS.UI.Tests` **3 202 grün** in beiden Kulturen, Warnungszahl unverändert. Herleitung und Messreihen im Protokoll `WindowsFormsApplication1/Allgemein/Reporting/iU9_W15a_Blazor_Port_Protokoll.md`, Abschnitt „W6‑B‑2‑O‑1 (07.09.2026)" |
...
| **W6‑O‑8** | **303 von 2 343 CEC-Geräten sind GELB — und alle aus demselben Grund.** Die Prüfung `WechselrichterPlausibilitaet` meldet „Die Kennlinie fällt im Teillastast" (η30 > η50) für 13 % des Auslieferungsbestands. Gemessen am 06.09.2026 über die volle Liste: Es ist kein Datenfehler, sondern die Modellparabel aus 3.3.3 — bei Geräten mit hohem Wirkungsgrad liegt ihr Scheitel zwischen 30 und 50 %, und genau das ist bei einem guten Stringwechselrichter auch physikalisch richtig. Die Warnung fragt jedes dieser Geräte beim Übernehmen zurück; das ist lästig und sagt nichts. **Empfehlung: Die Regel auf einen Schwellwert heben** (etwa: melden erst, wenn η30 − η50 > 0,01) oder sie auf handgepflegte Sätze beschränken. | **umgesetzt in `de5da29`** — Anwenderentscheid vom 06.09.2026, wörtlich: „Empfehlung". Die Regel meldet erst ab der benannten Konstanten `WechselrichterPlausibilitaet.TEILLAST_ABFALL_SCHWELLE = 0,01`, also **einem Prozentpunkt** Wirkungsgrad zwischen zwei benachbarten Stützstellen; der Grenzfall GENAU auf der Schwelle bleibt still. **Der Wert ist gemessen, nicht geraten** — über alle 2 343 Geräte der Auslieferungsdatei, Abfall `η_i − η_{i+1}` je Intervall: 5→10 % und 10→20 % **kein einziger** Abfall, 20→30 % zwei (größter 0,372 PP), 30→50 % dreihundertdrei (302 davon unter 0,4 PP, 246 sogar unter 0,1 PP), 50→100 % 2 019 (nicht geprüft, das ist der erlaubte Ast). **Zwischen 0,4 und 2,4 Prozentpunkten liegt über den ganzen Bestand keine einzige Kennlinie** — die Schwelle sitzt also in einer LÜCKE, nicht in einer Verteilung; jeder Wert darin ergäbe dasselbe Bild, 1,0 ist die runde Zahl. Der Meldungssatz nennt den Wert („… — ein Abfall von 2.423 Prozentpunkten"). **Neue Verteilung der Auslieferungsliste: 2 342 grün / 1 gelb / 0 rot** (vorher 2 040 / 303 / 0); gelb bleibt allein „OutBack Power: GS8048A {240V}" mit 2,423 PP zwischen 30 und 50 % — und genau dafür ist die Regel da. Ein Tippfehler wie 0,79 statt 0,97 sind 18 PP, das Achtzehnfache der Schwelle. Nachweise: drei Fälle in `WechselrichterKatalogTests` (Parabel bei 0,35 PP → grün, Grenzfall auf 1,0 PP → grün und bei 1,01 PP → gelb, Tippfehler → gelb mit der Zahl im Satz, Kultur gepinnt) und die neue Zahl in `CecWechselrichterAuslieferungTests` |
...
| **W6‑E‑6** | **Der Herstellerfilter über der Strangtabelle stand immer auf „Alle".** Wer Module eines Herstellers verbaut, der auch Wechselrichter baut, suchte sein Gerät jedes Mal von Hand aus dem ganzen Katalog — bei 152 Herstellern im CEC-Bestand ist das der überwiegende Teil der Klappliste. | **umgesetzt in `5762f32`** — Anwenderentscheid vom 07.09.2026, wörtlich: „Der Wechselrichter soll beliebig wählbar sein und nur die Vorauswahl auf den Modulhersteller verweisen (falls Wechselrichter von dem Modulhersteller verfügbar)." Der Filter steht beim Aufmachen auf dem Hersteller des ANLAGENmoduls, sofern der Wechselrichterkatalog ein Gerät dieses Herstellers führt — sonst auf „Alle"; er **sperrt nichts**, jeder andere Hersteller bleibt eine Klapplistenwahl entfernt, und eine schon gewählte Zeile behält ihr Gerät. Der Namensabgleich ist zweistufig (Gleichheit ohne Rücksicht auf Gross-/Kleinschreibung und Randleerzeichen, dann „beginnt mit" in beide Richtungen ab drei Zeichen); vorgestellt wird beim ersten Zeichnen und bei jedem WECHSEL des Anlagenmoduls, sonst nie — sonst nähme jedes Neuzeichnen dem Anwender seine Filterwahl ab. Die Trefferprüfung sitzt in `PvStraengeFelder` und fragt allein die Liste `Hersteller`, damit iOS denselben Weg hat. Eine Herleitungszeile unter dem Filter nennt die Vorauswahl und die freie Wahl. **Elf bunit-Fälle**, darunter die Gegenprobe zum zu kurzen Präfix und der Nachweis, dass ein fremdes Gerät nach der Vorauswahl wählbar bleibt und sich in die Zeile einträgt |
| **W16c‑E‑7** | **Der Menüweg zum Wechselrichter ist eine Stufe tiefer gerückt.** Das Paar Modul/Wechselrichter steht an ZWEI Stellen des Kopfes „Administration" — als Katalog unter „Energiesysteme" (S1.4, Kapitel 6) und als Import unter „Daten & Import" (5.2/5.5). Der Anwender wollte beide unter einem eigenen Knoten sehen: „Mache zwei Untermenüs Photovoltaik 1. PV Module 2. Wechselrichter", dazu die Richtigstellung „der Import steht nicht unter Energiesysteme sondern vdi3805 (Wechselrichter)". | **umgesetzt in `5a378d7`** — Anwenderentscheid vom 07.09.2026. Zwei Zwischenknoten **„Photovoltaik"** (`MenuItem_PV_Gruppe`, `MenuItem_PV_Import_Gruppe`, beide `MENU_PHOTOVOLTAIK`) in `EPOS.UI/Bausteine/Menuetabelle.cs`; **Ziel, Argument und Kennung der vier Punkte sind unverändert**, es wandert ihre Lage im Baum. Zwei Beschriftungen sind neu, weil der Knoten den Begriff schon trägt: „Photovoltaik Module" → **„PV Module"** (`MENU_PV_MODULE`) und „Import Photovoltaik CEC/Pan" → **„PV Module (CEC, PAN)…"** (`MENU_PV_IMPORT_CEC` — Wert geändert, Schlüssel gleich); beides in beiden Sprachen. Die Regel aus **W16c‑E‑6** („kein Untermenü mit nur einem Punkt") bleibt gewahrt: je zwei. Das Menüband klappt die neue Stufe **ohne Änderung** auf — die Pfadmechanik aus W16c‑B13 ist tiefenunabhängig, und `.epos-menueband-klappe--tief` gilt für jede Ebene ab der dritten. Nachweise: elf bunit-Fälle (zehn in `MenuebandTests`, einer in `HauptfensterTests`), Menüzahlen **58 Punkte / 44 handelnd / 14 aufklappend** |
