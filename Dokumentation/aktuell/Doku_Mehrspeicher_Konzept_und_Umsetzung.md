...
**Die Vorprüfung** (`FlottenPlausibilitaet.Pruefe`, [`EPOS.Kern/Controller/FlottenPlausibilitaet.cs`](../../EPOS.Kern/Controller/FlottenPlausibilitaet.cs)) läuft in `SpeicherFlottenStudieCtrl.Rechnen` vor der Rechnung und nach der Rechnung noch einmal mit der Diagnose. Sie liefert Hinweise mit Stufe (`Hinweis`/`Warnung`/`Problem`) und sprachneutraler Kennung; sie landen in der bestehenden Hinweisliste **und** zusätzlich strukturiert in `SpeicherFlottenErgebnis.Pruefhinweise`, an denen die Oberfläche ihre Abhilfeknöpfe aufhängt (Paket P3). Ein Befund der Stufe `Problem` **sperrt**: Die Ansicht sperrt daran ihren Rechenknopf, und `SpeicherFlottenStudieCtrl.Rechnen` weist den Lauf benannt ab, statt ihn in der Engine auflaufen zu lassen.
...
| `PrognoseFehlt` | planendes Betriebsziel (`PvPlanung`, `Arbitrage`, `MultiUse`) auf `PrognoseArt.VerifiziertBekannt`, ohne dass ein Snapshot dieser Art geladen ist (**Problem**) |
| `LebensdauerkurveUngueltig` | eine Einheit trägt einen Punkt der Lebensdauerkurve, mit dem die Rainflow-Auswertung nicht rechnen kann — nicht ausgefüllt, Wert ausserhalb des Bereichs oder doppelte Entladetiefe (**Problem**) |
...
**Die Prognosepflicht planender Ziele** (`FlottenPlausibilitaet.Prognosepflicht`). Die drei planenden Betriebsziele verlangen je Planungsschritt einen Prognose-Snapshot der eingestellten `PrognoseArt`; für `VerifiziertBekannt` — die Vorbelegung — nimmt der Kern ausschließlich archivierte Snapshots aus `SpeicherAuslegungKonfiguration.FlottenPrognosen` (CSV-Import „Prognosen und Projektjahre…"), denn die Istreihe wird nie still als historische Prognose verwendet (Spezifikation 9.4). Ist keiner geladen, greift die Regel und nennt beide Auswege: den Informationsstand auf `Oracle` („Idealwissen") setzen oder Prognosen laden. Geprüft wird allein die **Art**, nicht der Horizont — ob ein geladener Snapshot jeden Entscheidungszeitpunkt lückenlos abdeckt, entscheidet der Simulator mit der Reihe in der Hand. Dieselbe Funktion rufen drei Wege: `FlottenPlausibilitaet.Pruefe` (beide Stufen der Vorprüfung — die Regel braucht keine Zeitreihe), `SpeicherFlottenProjektCtrl.Pruefe` (Aktivierung und Projektlauf scheitern damit benannt, bevor gerechnet wird) und die Abweisung im Studienlauf. Die **Vorbelegung bleibt `VerifiziertBekannt`**: Sie ist die bewusste Wahl des Wissensstandes und wird nicht still umgestellt; die Abhilfe ist ein Knopf.

**Die Gültigkeit der Lebensdauerkurve** (`FlottenPlausibilitaet.Lebensdauerkurve`, `FlottenPlausibilitaet.Kurvenbefund`). Die Bedingung selbst steht in der Engine: `FlottenRainflow.PruefeKurve` liefert den ersten Mangel einer Kurve — Punktindex in der GELIEFERTEN Reihenfolge und Grund — oder `null` für eine gültige **und für eine leere** Kurve; `FlottenRainflow.Auswerten` ruft dieselbe Funktion und wirft unverändert `ArgumentException`. Gültig ist ein Punkt mit `Entladetiefe` in (0, 1] und `ZyklenBisEol > 0`, und jede `Entladetiefe` darf nur einmal vorkommen — zwischen den Stützstellen wird logarithmisch interpoliert, zwei Zyklenzahlen zu derselben Tiefe wären zwei Antworten auf dieselbe Frage. Ein im Einheiteneditor angelegter Punkt steht auf 0/0 und heißt deshalb „nicht ausgefüllt“: Das Modell führt `double`, `NaN` bräche die JSON-Serialisierung der Stände. Gerufen wird die Regel aus `FlottenPlausibilitaet.Pruefe` (beide Stufen der Vorprüfung — sie braucht keine Zeitreihe) und aus `SpeicherFlottenProjektCtrl.Pruefe`; die Abweisung des Studienlaufs und die Sperre des Rechenknopfs hängen generisch an der Stufe **Problem**. `Kurvenbefund` liefert dem Einheiteneditor Punktnummer (1-basiert) und denselben Wortlaut, mit dem er die betroffene Punktzeile markiert.
...
[Spezifikation](Spezifikation_Stromspeicher_Optimierung.md).
...
Der bestehende CSV-Dialog normalisiert 15- oder 60-Minuten-Reihen auf Viertelstunden und behandelt Zeit-/Wertspalten, Zeitzone, Intervalllage, Kodierung und Einheiten. Lücken, doppelte UTC-Stempel und nicht endliche Werte werden abgewiesen; negative Preise sind erlaubt. Eine CSV-Achse bleibt erhalten. Mischung mit EPOS-Modellwerten verlangt eine explizite Kalenderzuordnung. Details: [Datenquellen und CSV-Import](../ueberholt/Doku_Speicherauslegung_Kosten_Zeitreihen.md#datenquellen-und-csv-import).
...
[`Projekte/Konzept_Stromspeicher_Dialoge_EPOS-Plan.md`](Konzept_Stromspeicher_Dialoge_EPOS-Plan.md)
...
bestimmen…", „Netzladung erlauben" oder „Zur Betriebsführung"; trägt die Hinweisliste die
Kennung `PrognoseFehlt`, kommt „Informationsstand auf Idealwissen setzen" dazu. Jede Abhilfe
ist ein Delegat; fehlt er, fehlt der Knopf („kein Delegat ist kein Knopf").
