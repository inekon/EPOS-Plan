...
| ✔ U32 | Die Preisbasis ging als Regelkennung `ID_Umrechnung` mit; ohne Regel nach kWh wurde −1 abgelegt, und die Wahl kWh fiel beim nächsten Öffnen auf die Abrechnungseinheit zurück (Rest von ET-D-3) | **umgesetzt #446** (E7c2, Schemaschritt 112): eigene Spalte `energy_project_settings.Preisbasis`, einmalig aus `ID_Umrechnung` befüllt (Testdatenbank: 5 Zeilen „kWh", 23 mit der Abrechnungseinheit); Preise und Kosten unverändert — Proben 1030 (Erdgas E mit Regel nach kWh) und 1024 (Stadtgas ohne Regel) mit Basis kWh gleich |
| ✔ U-1 | Einheitenbruch: `Tab_Brennstoff_Stamm.Einheit` der fünf Gase „m³" gegen `energy_carrier.billing_unit` „Nm³" — die Identitätsregel-Ableitung lieferte −1 | **umgesetzt #446** (E7c2, Schemaschritt 113, reines DML; A9): Einheit und Preiseinheit der fünf Gase auf Nm³, dazu eine Preiszeile (Projekt 1039); der Brennstoff 24 „Sonstige" auf kWh (E7c2‑Q4, Anwender) — als Einheitentext, sein Stamm trug damals H_i = H_s = 0; Energiekosten und Emissionen der Basisprojekte unverändert, der Vorlagenbau läuft ohne Auffälligkeit. **H_i = H_s = 1,0 für „Sonstige" umgesetzt #452** (E7c3/6): die Katalog-Generation 9 pflegt den Stamm einmal nach, nur wo beide Werte noch 0 oder leer sind, ohne Schemaschritt; die Testdatenbank ist nachgepflegt (E7c3/12), kein Basisprojekt nutzt den Brennstoff. Das Beispiel dieses Papiers schreibt die Gasmenge weiter in m³ |
