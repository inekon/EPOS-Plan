...
| `Pruefung/Katalogprobe.cs` | die **Katalogprobe** des Prüfmodus: nur mit zusätzlich `EPOS_PRUEFLAUF_KATALOGIMPORT`, hinter dem Rechennachweis. Das erfundene Probepaket der Brauchwasser-Nutzungsarten (`EPOS.Kern.Tests/Proben/Zapfprofil/Katalogpaket/`, sieben CSV, rund 3 KB) liegt als `MauiAsset` mit Präfix `katalogprobe_` in **jedem** Paket; die Hülle packt es zur Laufzeit zu einem ZIP-Archiv, gelesen, geprüft, eingespielt und gezählt wird im Kern (`TwwKatalogprobe.Probelauf`, Zeilen `KATALOGPROBE`). `ios.yml` setzt den Schalter im Hauptlauf und wertet die Zeilen im Schritt „Katalogprobe auswerten“ aus. Der Katalog der Brauchwasser-Nutzungsarten ist der einzige Katalog, den `IosProjektQuelle` führt (`NutzungsartKatalogGaben`) |
...
| `IDateiDienst` | `IosDateiDienst` | `FilePicker`, `Share` | `DateiSpeichern` liefert einen Pfad unter `Documents`; `OrdnerWaehlen` = `""`, `OrdnerwahlMoeglich` = `false` — wer einen Ordner ODER eine Datei nimmt (Katalogimport der Brauchwasser-Nutzungsarten), bietet hier allein das ZIP-Archiv an |
