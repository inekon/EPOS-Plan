...
{
    public partial class Form_Kosten_VarAuswahl : Form
    {
        // Diese Eigenschaften liest das Hauptprogramm nach dem Schließen aus
        public string SelectedName => comboBox_Varianten.Text;
        ...
        public string m_szBrennstoff { get; set; } = ""; // Optional: Vorgabe für die Brennstoffart (Bezeichner) in der ComboBox

        public int m_KategorieID { get; set; } = 0;  

        public string m_szKategorie { get; set; } = ""; // Optional: Vorgabe für die Kategorie (Bezeichner) in der ComboBox 

        public Form_Kosten_VarAuswahl()
        {
            InitializeComponent();
        }
        ...
        private void LoadBrennstoffArten()
        {
            ...
            string sql = "SELECT ID, Bezeichner FROM Tab_Brennstoff_Stamm where ID_Kategorie = " + m_KategorieID + " ORDER BY Bezeichner";
            ...
            comboBox_Varianten.DataSource = DataRepository.GetDataTable(sql);
            comboBox_Varianten.DisplayMember = "name";
            comboBox_Varianten.ValueMember = "ID";
            comboBox_Varianten.Text = m_szBrennstoff;   
        }

        private void btnOk_Click(object sender, EventArgs e)
        {
            if (string.IsNullOrWhiteSpace(comboBox_Varianten.Text))
            {
        ...
        {
            comboBox_Varianten.Text = cmbBrennstoffArt.Text;
        }

        private void Form_Kosten_VarAuswahl_Load(object sender, EventArgs e)
        {
            LoadBrennstoffArten();
        }
    }
