...
|---|---|---|
| `Solarthermie/SolarkollektorenDialog` | `Form_SolarKollektoren`: links „ausgewählt im Projekt", rechts „aus Datenbank", dazwischen ◀/▶, darunter der Detailblock | Stand schon nebeneinander (`epos-auswahlpaar`), jetzt `Zweispaltenauswahl`; die Katalogknöpfe bleiben unter der Katalogliste in der rechten Spalte, der Detailblock unter dem Paar über die volle Breite |
| `Solarthermie/SolarganglinieDialog` | `Form_Solarganglinie`: zwei Listen, dazwischen ◀/▶, „Bearbeiten…" unter der Katalogliste | `Zweispaltenauswahl`, unverändert in der Aufteilung |

**Nicht betroffen sind die zwei Wärmepumpenmasken und der Kennlinieneditor.**
`WaermepumpeStammDialog`, `WaermepumpeAnlageDialog` und `KennlinienEditorDialog` nutzen
zwar `epos-auswahlpaar`, aber **ohne** Pfeilspalte: Dort stehen zwei gleichrangige
Blöcke nebeneinander (Stammdaten und Kennlinien bzw. Eingabe und Ergebnis), keine
Projekt↔DB-Auswahl. Für sie ist ein Umbruch folgenlos, sie bleiben auf dem alten Block.
Ebenso `WaermepumpenDialog` (`Form_WPAuswahl`) und `WaermepumpenKatalogDialog`: **eine**
Liste, kein Paar.
