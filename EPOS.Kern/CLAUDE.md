...
| `Allgemein/Update/` (6) | `Anlagenzeilen`, `ProjektPuffer`, `SchemaKatalog`, `SchemaStand` (Ergebniszustand der Migration und die DDL-Konstanten, die Controller zur Selbstanlage brauchen; **`Zielversion` steht seit dem 07.09.2026 auf 68** — Schemaschritt 68 legt `Firma` in `Tab_Stromspeicher_STAMM` und der Projektkopie `Tab_Stromspeicher` an, Anwenderentscheid **W14a‑E‑10‑Q7**: `Tab_Stromspeicher_STAMM` war der EINZIGE Gerätekatalog ohne Herstellerspalte, und eine Spalte, die es in der Tabelle nicht gibt, kann man nicht sortieren), **`StromspeicherFirmaNachtrag`** (die DML dazu: der Text vor dem ersten Doppelpunkt des Bezeichners, idempotent — Muster der Schritte 57/58/67; was kein Präfix hat, bleibt leer, der Nachtrag rät nicht) — seit iU5-U5 dazu `AnlagenEindeutigkeit`, seit iU9‑W10a `ProjektPuffer.NutzbareKapazitaetKWh` (Volumen × 1,16 × Spreizung ÷ 1000 — die Formel stand in ZWEI Masken, Befund W10‑B12; die Leerregeln bleiben je Maske) |
