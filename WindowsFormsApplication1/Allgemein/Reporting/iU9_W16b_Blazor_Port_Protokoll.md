...
| **W16b‑O‑2** | ~~**`ProjektTransferDialogTests.Schliessen_meldet_ob_ein_Import_gelungen_ist` ist flatterhaft** (Befund W16b‑B7) — nicht von dieser Welle verursacht, aber im Gesamtlauf einmal rot~~ — **erledigt 06.09.2026**: **zwei Wettläufe im TEST**, kein Fehler des Dialogs. Der Fall wartete auf das Kennzeichen der Attrappe (`kern.ImportGerufen`), das im `Task.Run` des Dialogs schon beim **Betreten** des Imports fällt, und klickte „Schließen", solange `_laeuft` noch stand — ein laufender Import meldet aber nichts; dazu kehrt bunits **synchrones** `Click()` zurück, ohne auf den Ereignisbehandler zu warten. Der Test wartet jetzt auf den **gezeichneten** Abschluss (Berichtsfeld) und fasst die Ergebnisprüfungen in `WaitForAssertion`. Herleitung, Messungen und die Durchsicht aller 28 Wartestellen im Protokoll `iU9_W15a_Blazor_Port_Protokoll.md`, Abschnitt „W16b‑O‑2 (06.09.2026)" |
