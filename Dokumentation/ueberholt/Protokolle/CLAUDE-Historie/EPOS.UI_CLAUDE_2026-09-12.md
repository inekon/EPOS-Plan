> Archivierte Fassung der `EPOS.UI/CLAUDE.md` vom 12.09.2026, vor der Kürzung auf den gültigen Stand. Die relativen Verweise sind auf diesen Ablageort umgeschrieben; Zählungen, Daten und Kürzel beschreiben den Stand von damals.
...
| `Projekt/ProjektVarianteDialog` | kein Vorläufer — er löst die generische `NamensDialog`-Abfrage im Ablauf „Als Variante speichern“ ab (Auftrag **#237**, Anwenderwunsch 12.09.2026) | Datenseite **plattformfrei** in `EPOS.UI.Daten/Projekt/ProjektVarianteHuelle.cs` (Regel #208) → `VariantenCtrl.Zielname`/`AnlegenAusStamm(…, idQuelle)` und `ProjektCtrl.NamenListe`; `WindowsFormsApplication1/Views/Varianten/AlsVarianteHuelle.cs` ist nur noch der Windows-Adapter (Fenster, Meldungen, das Nachziehen der Startseite als Rückruf). **ZWEI Wege führen zu demselben Dialog** — der Menüpunkt „Als Variante speichern…“ und der Eintrag im Auswahlfeld „Projekt:“ des Kopfbands (`StartseiteHuelle.VarianteAnlegenJetzt`, Anwenderwunsch 08.09.2026); sie unterscheiden sich NUR in der Erfolgsmeldung (Meldungskasten gegen Kurzhinweis der Seite), und deshalb steht das Fenster einmal in `AlsVarianteHuelle.Fragen`. Ein Kontrollkästchen blendet die `ProjektListe` ein, aus der die Variante ihren INHALT bekommt; der NAME bleibt „&lt;Stamm&gt; - &lt;Bezeichner&gt;“ und `Tab_Variante.ID_ProjektRef` zeigt weiter auf den Stamm. **Der VON-HAND-MERKER ist der Teil, den man lautlos falsch baut:** Die Projektwahl belegt den Bezeichner mit dem Projektnamen vor — aber nur, solange der Anwender nicht selbst getippt hat; sonst löscht jeder Auswahlwechsel seinen Text weg (anders als in `ProjektKopieDialog`, wo ein Wechsel die drei Verwaltungsfelder bewusst überschreibt). Den ZIELNAMEN zeigt er live aus DERSELBEN Kernregel, statt „&lt;Stamm&gt; - &lt;Bezeichner&gt;“ samt Kollisionszähler nachzubauen (`Func<string,string> Zielname`). Titel nach W11b‑B‑9 (leerer `TitelText` = kein eigener `<h1>`); die generische `NamensDialog` bleibt unverändert, sie bedient vier weitere Masken |
