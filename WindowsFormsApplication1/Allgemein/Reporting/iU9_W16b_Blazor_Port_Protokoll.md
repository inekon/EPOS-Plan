...
| **W16b‑O‑2** | ~~**`ProjektTransferDialogTests.Schliessen_meldet_ob_ein_Import_gelungen_ist` ist flatterhaft** (Befund W16b‑B7) — nicht von dieser Welle verursacht, aber im Gesamtlauf einmal rot~~ — **erledigt 06.09.2026**: **zwei Wettläufe im TEST**, kein Fehler des Dialogs. Der Fall wartete auf das Kennzeichen der Attrappe (`kern.ImportGerufen`), das im `Task.Run` des Dialogs schon beim **Betreten** des Imports fällt, und klickte „Schließen", solange `_laeuft` noch stand — ein laufender Import meldet aber nichts; dazu kehrt bunits **synchrones** `Click()` zurück, ohne auf den Ereignisbehandler zu warten. Der Test wartet jetzt auf den **gezeichneten** Abschluss (Berichtsfeld) und fasst die Ergebnisprüfungen in `WaitForAssertion`. Herleitung, Messungen und die Durchsicht aller 28 Wartestellen im Protokoll `iU9_W15a_Blazor_Port_Protokoll.md`, Abschnitt „W16b‑O‑2 (06.09.2026)". **Zweite Fundstelle am 07.09.2026 (`W6‑B‑2‑O‑1`):** Der Kern-CI-Lauf 216 fiel an `ModulImportDialogTests.Der_Herstellerfilter_zeigt_nur_noch_die_Zeilen_des_Herstellers` aus („Expected 5, Actual 155"), und es war derselbe zweite Wettlauf — nur allgemeiner: Er trifft JEDES synchrone bunit-Ereignis (`Click()`, `Change()`, `Input()`), sobald der Zeichnerfaden gerade besetzt ist. Seither lautet die Hausregel: **nach einem synchronen bunit-Ereignis auf den GEZEICHNETEN Zustand warten, nicht sofort prüfen.** Herleitung, Messreihen (68 von 400 rot unter Last, 0 von 400 danach) und die nachgezogenen Wartestellen beider Importklassen im selben Protokoll, Abschnitt „W6‑B‑2‑O‑1 (07.09.2026)" |
