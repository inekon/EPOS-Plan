...
| `Zeilenwahl` | Der runde Wahlknopf einer Rasterzeile (`aria-pressed`, 44 px); seit iU9‑W13.0l mit **Mehrfachmodus** — `Mehrfach` macht daraus ein Kontrollkästchen (`role="checkbox"`), und `Tastenwahl` meldet `Strg`/`Umschalt` mit. Seit iU9‑W4 (W4‑E‑1) dazu die **breite Zeile**: `Beschriftung` macht aus dem runden Rasterziel eine volle, linksbündige Listenzeile mit dem Zeichen vorn (`epos-zeilenwahl--breit`), `Zusatzklasse` hängt eine Klasse des Wirtes an. Für schmale Auswahlspalten, in denen die **Zeile ihren Namen selbst trägt** statt neben einer Namensspalte zu stehen (Trägerliste der Energieträgerverwaltung) — **ein** Klickziel und **ein** Tabulatorhalt je Zeile. Ohne `Beschriftung` bleibt das Markup unverändert | die Zeilenmarkierung von `ListView`/`DataGridView`, die ein `Raster` nicht kennt |
