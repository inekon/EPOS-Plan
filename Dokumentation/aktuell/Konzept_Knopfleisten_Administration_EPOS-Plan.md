...
**Stand:** Konzept liegt vor, Umsetzung nach Abnahme der Mockups.
...
|---|---|---|---|---|---|---|
| 1 | `Bedarf/GebaeudeDialog.razor` (Gebäude → Bearbeiten) | Katalogspalte: Ändern… · Neu… · Löschen · Gebäudetyp…; Detailblock: Ändern · Simulation… (nur Projekt); Fuß: OK · Abbrechen | **Verwaltung:** Katalogspalte Neu… · Ändern… · Löschen (bleibt, Listenleiste); Fuß **Füller · Beenden**. **Projekt:** Fuß **Ändern… · Simulation… · Füller · Abbrechen · OK**; Detailblock ohne Knöpfe. „Gebäudetyp in DB ändern…" → DL-Q1 | OK/Abbrechen der Verwaltung waren eine Behauptung (jede Katalogaktion schreibt sofort); im Projekt wandern zwei Knöpfe vom Detailblock in den Fuß | M | Die Gebäudeverwaltung schließt mit „Beenden"; im Projektdialog stehen „Ändern" und „Simulation" in der Schlussleiste. |
...
- **Navigierende Knöpfe** (Grafik…, Kennliniendaten…, Typ ändern…, Simulation…, Gebäudetyp…) öffnen
  ...
  ändern…" hat in der Verwaltung einen Zwilling im Menü (Gebäude → Gebäudetypen) — DL-Q1.
...
## 3 Offene Anwenderentscheide

| Kennung | Frage | Variante (a) | Variante (b) | Empfehlung |
|---|---|---|---|---|
| **DL-Q1** | Gebäudeverwaltung: bleibt „Gebäudetyp in DB ändern…" in der Katalogspalte? | Nur im **Projektdialog** behalten (dort gibt es kein Menü daneben); in der Verwaltung streichen, der Menüpunkt „Gebäudetypen" steht direkt darunter | In beiden Betriebsarten behalten, links vom Füller | **(a)** — ein Weg je Ziel; die Verwaltung wird um einen Knopf leichter |
...
| **DL-Q3** | Kosten und Energieträger: Reihenfolge jetzt oder voller Arbeitsstand? | **Reihenfolge nach Hausregel** (`SpeichernLeiste` mit `MitSpeichern`); Zeilenaktionen (Position anlegen, Übernahme, Katalog, Neu, Variante, Löschen) schreiben weiter sofort, weil sie Ids brauchen; Abbrechen verwirft die ungespeicherte Karte bzw. die ungespeicherten Eingaben, und der Kurztext des Knopfes sagt das | **Voller Arbeitsstand:** alle Zeilenaktionen sammeln bis OK, die Hüllen (1 336 und 2 773 Zeilen) bauen ihren Schreibweg um — Aufwand L je Dialog, eigenes Konzept | **(a)** für DL-2; (b) als eigenes Konzept vormerken, wenn die Kostendialoge ohnehin angefasst werden |
| **DL-Q4** | Importdialoge (8, 9): Wer trägt die Primärfarbe? | **Beenden** (Hausregel: der Schlussknopf ist der einzige primäre); „Auswahl übernehmen" steht davor, gesperrt bis eine Zeile gewählt ist | **Auswahl übernehmen** bleibt primär (heute) — Ausnahme von der Regel für Importmasken | **(a)** — eine Regel ohne Ausnahme; die Übernahme ist über die Sperre und den Doppelklick auffindbar genug |
| **DL-Q5** | Energieträger: „Stammwerte speichern" in Speichern/OK aufgehen lassen? | **Ja:** Bezeichnung und Gruppe sind Felder der Karte; Speichern/OK schreiben sie mit, der Knopf entfällt | Nein, der Knopf bleibt in der Kartenspalte | **(a)** — zwei Speichern-Knöpfe in einer Maske sind die einzige echte Doppelung des Inventars |
| **DL-Q6** | Einstellungen: Der OK-Knopf heißt „Speichern" (heute) oder „OK"? | **„OK"** — ein Wort für einen Weg (prüft, speichert, schließt), wie in jedem anderen Dialog | „Speichern" bleibt (Wortlaut des Vorläufers) | **(a)**; Aufwand ein Ressourcentext |


## 4 Umsetzungsreihenfolge

Je Komponente ein Auftrag (DL-2a … DL-2k), kleine und risikolose zuerst; jeder Auftrag ist erst
nach der Abnahme seines Mockup-Abschnitts frei.

| Schritt | Auftrag | Aufwand | Voraussetzung |
|---|---|---|---|
| 0 | `SpeichernLeiste`: Aktionsschlitz links (`Aktionen`), bunit-Test; dazu die Wache **`KnopfleistenWacheTests`**: Jede Fußleiste unter `EPOS.UI/Dialoge/**` trägt genau einen `epos-knopf--primaer`, und er ist der letzte Knopf; wo `Abbrechen` steht, steht es unmittelbar vor ihm | S | — |
...
- Wache aus Schritt 0 grün über den ganzen Dialogbestand.
