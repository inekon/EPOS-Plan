...
- **Die Namensregel und der Weg zur Maske.** Tolerant gelesen wird nur der **Wortlaut des Modells**, und zwar nach
  ...
  etwas findet, entscheidet, und mehrdeutig bleibt mehrdeutig (die Absage nennt die Kandidaten). So liest die offene
  Maske einen Feldnamen (`KiMaskenbruecke.Feldsuche`), und so wird ohne offene Maske die GEMEINTE Maske aus den
  Feldnamen erschlossen — zuerst über Schlüssel, erst ohne Treffer tolerant; führen die Treffer an verschiedene Orte,
  nennt die Absage keine Maske, sondern die Liste. **Zwischen zwei Schlüsseln gibt es keine Nachsicht:** Führt die
