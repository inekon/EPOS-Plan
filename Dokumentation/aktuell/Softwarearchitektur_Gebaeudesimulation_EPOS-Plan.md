...
[ADR-002](ADR-002_Stundenmodell_VDI6007_Einbindung.md) (angenommen),
[ADR-003](ADR-003_IFC_xBIM_ohne_Geometriekernel.md) (angenommen),
[ADR-004](ADR-004_gbXML_LINQ_to_XML.md) (vorgeschlagen),
[ADR-005](ADR-005_Zonenkopplung_Mehrzonenmodell.md) (vorgeschlagen) und die Hausregeln der vier
`CLAUDE.md` ([Wurzel](../../CLAUDE.md), [`EPOS.Kern`](../../EPOS.Kern/CLAUDE.md),
[`EPOS.UI`](../../EPOS.UI/CLAUDE.md), [`EPOS.iOS`](../../EPOS.iOS/CLAUDE.md)).

Die Anwenderentscheide **E1–E11** (Konzept Nachtrag 1,
[Statusdatei](Status_Gebaeudesimulation_VDI6007.md)) sind verbindlich und werden nicht neu
verhandelt. **E11** ist der jüngste — ein Gebäudebetrachter mit zwei Ansichten auf **einem**
Zonengeometrie-Modell (Konzept N1.16); der Konzepttext überlässt die endgültigen Namen
ausdrücklich diesem Papier. Seine Architekturfolgen stehen in 1.2, 1.3, 3.2, 3.4, 3.6, 3.8, 4.5,
4.6 und in Kapitel 5.

---

## 0. Das Ergebnis in sechs Punkten

1. **Die Physik und die Zonengeometrie liegen in einer Ablage mit vierzehn Bausteinen** —
   ...
   damit von selbst unter `DoubleWacheTests`, das über Unterordner von `Simulation/` greift. Die
   zwei letzten Bausteine sind das **Zonengeometrie-Modell** nach **E11** (`Zonengeometrie`,
   `Zonenumriss`, Stufe G6c): **eine** Quelle, **drei** Abnehmer — die Ansicht, der gbXML-Export
   und der IFC-Export.
...
3. **Das Datenmodell wächst um 15 + 3 Spalten und elf Tabellen** — neun für Baustoffe, Aufbauten,
   Schichten, Zonen, Bauteile und Luftströme, zwei für die Importherkunft —, bei **einem**
   Sichtneubau, sobald **A9 = U5** die beiden Gebäudespalten-Schritte verschmilzt; bleiben sie
   getrennt, sind es zwei. Der Klimaschritt bleibt in beiden Fällen getrennt und berührt die Sicht
   nicht. Die Ergebnisreihen bleiben draußen: Sie gehen als CSV und Skalare aus dem Lauf,
...
5. **Die Integration berührt sieben Stellen, jede benannt:** die Verzweigung im Lauf (zwei Aufrufer
   — `SimulationWaermebedarf.cs:197` für den Lauf, `GebaeudeBedarfCtrl.cs:117` für die Auskunft —
   und zwei Verzweigungspunkte: `SimulationWaermebedarf.cs:581` in der Methode selbst und `:647`
   in der Flächenrückrechnung, die ihr **vorausläuft**),
   den Bedarfsdialog, Bericht und Kennzahlen, den Referenzlauf-Export, `kern.yml`, den
   iOS-Prüfmodus und die Lizenzhinweisseite des Installationspakets.
6. **Drei Schritte frieren die Referenzbasis neu ein** — GB, G1 + G2 und G6d —, und **neunzehn
   Architekturfragen A1–A19** liegen beim Anwender. Ohne A7 (ADR-004) hat der gbXML-Import keinen
   Leseweg, ohne A8 (ADR-005) das Mehrzonenmodell keine Kopplung.
...
flowchart TB
  ...
  subgraph FREI["Plattformfrei (EnableWindowsTargeting=false)"]
    ...
    KSCH["EPOS.Kern/Allgemein/Update/<br/>6 Schema-Klassen"]
  end
  REF["EPOS.Referenzlauf · Referenzlauf<br/>3 bedingte Reihen je Gebaeude"]
  DB[("Kenndaten.sqlite<br/>15+3 Spalten · 11 Tabellen · 1 Sicht")]
...
handgeschriebenem Lesemodell (ADR-004, Vorschlag; A7). Die **Oberfläche** nimmt für die Körperansicht
...
**Vier Festlegungen, die aus dem Bestand folgen.**
...
- **`Zuruecksetzen` ist Pflicht, keine Bequemlichkeit.** Ein Verbrauchsgebäude wird **zweimal**
  gerechnet (Umsetzungskonzept 1.5); die Gegenprobe „zwei Läufe byte-gleich" gehört als Rechenprobe
  dazu.
...
2. **Die Größengrenze ist kein Glied der Plattformnaht, sondern ein Datum des Profils.** Sie
   ...
   20 MB iOS (U11), gbXML 25 MB Windows / 10 MB iOS (D15); **beide iOS-Zahlen sind zu messen, nicht
   zu schätzen**. **Diese vier Zahlen stehen allein hier** — der Systementwurf verweist auf diese
...
| **Zweimal rechnen muss gehen** | `Zuruecksetzen(thetaStart)` vor jedem Lauf; die Verbrauchsrückrechnung in `Bewohner_und_Flaeche_berechnen` ruft den Rechenweg ein zweites Mal (`SimulationWaermebedarf.cs:647`) — und zwar **vor** dem Verzweigungspunkt `:581` der Bedarfsrechnung (4.1) | Sonst mischt die Verhältnisrechnung zwei Zustände, und die zweite Zahl hängt an der ersten |
...
1. **Verbindungsbesitz.** Der Ablauf **liest** die Datei im Arbeitsfaden und berührt die Datenbank
   ...
   läuft auf dem Faden des Aufrufers über `DataRepository` wie jeder andere Schreibweg.
...
Sechzehn prüfbare Sätze in fünfzehn Nummern — AR8 zerfällt in zwei Hälften mit verschiedenen
...
**Drei Wächter kommen hinzu, ein vorhandener bekommt neue Einträge** — ohne sie hat die Architektur
vier Stellen ohne Halt:
...
Hinzu kommen **15 Spalten** an `Tab_Gebaeude(_STAMM)`, **drei** an `Tab_Solar(_STAMM)` und **elf
Tabellen**: neun für Baustoffe, Aufbauten, Schichten, Zonen, Bauteile und Luftströme, zwei für die
Importherkunft.
...
erDiagram
    ...
    Tab_Gebaeude ||--o{ Tab_DBTagV : "Bestand, Eingang des Tagesmodells"
...
erDiagram
    ...
    Tab_Importzuordnung }o--o| Tab_Gebaeude : "ID_Gebaeude, OHNE Kaskade"
...
> **Unverändert:** `Tab_Projekt`, `Z_ProjektGebaeude`, `Tab_Klimaregion`, `Tab_Klimadaten`,
...
Eltern. Die Zahl der `STRICT`-Tabellen ist zugleich die Größe, an der der Prüfmodus der iOS-Schale
...
|---|---|---|---|---|
| `Gebaeude_Modell` | `TEXT(20)` | TEXT | **`VDI6007`** (E1, W12) | G1 |
...
Tagesbilanz-Wegs; die zwei neuen Spalten sind die Auflösung nach Himmelsrichtungen, und ihr NULL
...
**Die drei neuen Spalten an `Tab_Solar` und `Tab_Solar_STAMM`** (`SolarKlimaSchema`, eigener
Schritt): `Gegenstrahlung`, `Windgeschwindigkeit`, `Luftfeuchte`, je `REAL`. Die Werte liegen in der
PVGIS-Antwort vor und werden heute verworfen. **Bestandsregionen bleiben NULL, bis der Anwender neu
importiert** — diese Zusage gehört in den Schrittbericht; der Schätzweg für die Gegenstrahlung steht
im Umsetzungskonzept 1.7 und läuft im `GebaeudeKlimaweg` mit einer Warnung über
`SimulationProtokoll`.
...
`ID_Bauteil`, `ID_Aufbau`, `ID_Baustoff` (alle **ohne** Kaskade); `Quellkennung`
...
| **W13** | Die zwei Gebäudespalten-Schritte des Konzepts werden **zu einem verschmolzen** (Papiername **M3**): 15 Spalten je Tabelle, 30 Einträge, **ein** Sichtneubau; die drei Klimaspalten bleiben ein eigener Schritt (**M4**). Vorbehalt: A9 = U5. Die Zahlen 77 und 78, die das Konzept dafür nennt, sind inzwischen anderweitig vergeben (2.4) | G1 und G2 werden nach E1 gemeinsam ausgeliefert; zwei Sichtneubauten hintereinander sind zwei Gelegenheiten, die Definitionen auseinanderlaufen zu lassen |
...
| **W18** | Drei neue Einfrierregeln, **benannt nach ihrem Gegenstand statt nach einer Ordnungszahl**: „gesäte Gebäudedaten" wird im Schritt **GB** eingetragen, „gesäte Klimareihen" (`Tab_Solar.Gegenstrahlung`, `Windgeschwindigkeit`, `Luftfeuchte`) im Schritt **M4**, „gesäte Zonendaten" im Schritt **G6d** — jeweils an **beiden** Orten: [`Referenzlaeufe/LIESMICH.md`](../../Referenzlaeufe/LIESMICH.md) und Abschnitt „Regressionsnetz" der [Wurzel-`CLAUDE.md`](../../CLAUDE.md) | Eine Regel, die später nachgetragen wird, schützt die Saat nicht, die sie schützen soll. Die Klimaregel ist neu gegenüber Befund V: M4 ist byte-gleich **nur, solange kein Leser die Klimaspalten liest**; ab G1 liest sie `GebaeudeKlimaweg`, und ein späterer PVGIS-Neuimport einer Referenzregion verschöbe die Basis sonst still. **Ordinalzahlen stehen bewusst nicht dabei:** M4 läuft lange vor G6d, und wer beide Regeln durchzählte, trüge in `Referenzlaeufe/LIESMICH.md` eine Nummernlücke ein |
...
| **W20** | Die Schema-Klassen heißen `GebaeudeSchema`, **`SolarKlimaSchema`**, `BaustoffSchema`, **`BauteilaufbauSchema`**, `ZonenSchema`, `ImportzuordnungSchema`. Befund V R1 nennt `BauteilSchema` und kennt `SolarKlimaSchema` nicht — R1 wird hier berichtigt | Die Klasse trägt **Aufbau und Schicht** (W5), nicht das Bauteil; das Bauteil liegt in `ZonenSchema`, weil es mit der Zone in **einem** Schritt entsteht. Der Klimaschritt hat andere Wirkung, anderen Mitläufercode und anderes Risiko und bekommt deshalb eine eigene Klasse |
...
| **M3** | `GebaeudeSchema` | 15 Spalten × 2 Tabellen, Sicht `DROP` + `CREATE`, Leser auf **Namenszugriff**, `DbWerte` | **ja**, solange kein Leser rechnet | keine — Bestandszeilen bleiben NULL (= Vorgabe) | läuft ohne Handgriff mit |
...
| **M4** | `SolarKlimaSchema` | 3 Spalten × `Tab_Solar(_STAMM)`, Leseweg der PVGIS-Antwort, Einfrierregel **„gesäte Klimareihen"** | **ja** | keine — Zusage in den Schrittbericht | `Tab_Solar_STAMM` bleibt vollständig (kein `ReadOnly`) |
...
| **S-C** | `ZonenSchema.Anweisungen` | `Tab_Zone` + `Tab_Bauteil`, Indizes `(ID_Gebaeude, Rang)` und `(ID_Zone, Rang)` | **ja**, solange kein Rechenweg liest | keine | — |
...
Der Zielstand des SQLite-Zweigs steht heute auf **78**
(`EPOS.Kern/Allgemein/Update/SchemaStand.cs:106`): Schritt 77 ist die Volumenbemessung der
Pufferspeicher-Vorlage, Schritt 78 der feste Betrag der PV-Position „Batteriespeicher", beide vom
15.09.2026. Die nächste freie Nummer ist damit **79**; Befund V geht noch von 76 aus, das Konzept
und das Datenaustauschkonzept nennen 77 und 78 für die Gebäudespalten — an dieser Stelle sind sie
überholt. Verbindlich sind **Reihenfolge und Inhalt** je Schritt; die Nummer vergibt der Schritt bei
seiner Beauftragung (**A11**), weil jede vergebene Nummer ältere Projektpakete entwertet — der
Schemastand steht im Transportmanifest.
...
ergebnisneutralen Tabellenschritt in G6b oder einen Einfrierschritt in G1.
...
leere Parameterfeld zeigt die Vorgabe an — im Feld als Platzhalter (A5 = U3) oder als
...
| **je Zeile** | `Herkunft` + `Quellkennung` an Zone, Bauteil, Aufbau, Baustoff | **dauerhaft** | Nach dem Lauf beantwortet sie „woher stammt diese Zone" und trägt den Rückweg des Round-Trips |
| **je Paarung** | `Tab_Importzuordnung` — eine Zeile je EPOS-Zeile ↔ Quellentität | dauerhaft | Der zweite Import derselben Datei erkennt, was er schon zugeordnet hat; der Round-Trip findet die Entität wieder |
...
der Zonencontroller nicht kennt.
...
umgestellt wird. Alle Tabellen-, Saat- und Sichtschritte sind ergebnisneutral — **und das ist
nachzuweisen, nicht zu behaupten** (R13).

```mermaid
stateDiagram-v2
    [*] --> Basis_R7 : aktuelle Basis, dreizehn Projekte
    Basis_R7 --> Basis_GB : GB — Instanzzustand, Ferienmaske, 100er-Grenze<br/>NEU EINFRIEREN, Regel Gebaeudedaten
    Basis_GB --> Basis_GB : M2 Umbenennung · M3 Gebaeudespalten · M4 Klimaspalten<br/>byte-gleich, Regel Klimareihen mit M4
...
| **M3** | byte-gleich; die Probe **ist** der Sichtneubau: Der Leser liefert alle 58 Bestandsfelder unverändert; `SqlDialektPruefer` grün |
| **M4** | byte-gleich; die Importprobe gegen die eingefrorene PVGIS-Antwort liefert dieselben Strahlungsreihen wie bisher; **Einfrierregel „gesäte Klimareihen" eingetragen** |
| **G1 + G2** | alle dreizehn Projekte ändern sich; Basis vollständig neu. Dazu der **Rückweg-Test** auf einer gitignorierten Arbeitskopie, die alle Gebäude auf Tagesbilanz setzt, gegen die GB-Basis; `ChartProben` grün; Sichtabnahme Windows |
...
- **„gesäte Klimareihen"** (mit M4): `Tab_Solar.Gegenstrahlung`, `Windgeschwindigkeit`,
  `Luftfeuchte` der Referenzregionen. Ohne sie verschiebt ein späterer PVGIS-Neuimport die Basis
  still, sobald `GebaeudeKlimaweg` die Spalten liest;
- **„gesäte Zonendaten"** (mit G6d): Zonen-, Bauteil-, Schicht- und Luftstromzeilen des umgestellten
  Referenzprojekts.

**Die Reihenfolge ist die der Eintragung, und sie trägt bewusst keine Ordnungszahlen.** M4 läuft
...
**Der Stand der Tagesmodell-Eingaben.** `Tab_DBTagV` und `Tab_DBTagVDaten` bleiben unverändert: Sie
sind der Eingang des **ausdrücklich wählbaren** Tagesbilanz-Wegs (E1) und liegen zu allen
Testgebäuden vor. Sie werden nicht eingefroren, nicht abgelöst und nicht erweitert — und der
Stundenzweig liest sie **nicht** (4.1).
...
| Gebäudeimport (IFC, gbXML) | **offen, A17** | bei einem Schlüssel: `AppWurzel.razor` | offen | Empfehlung: **keiner** — Überlagerung im Gebäudedialog, aufgemacht über zwei Knöpfe in der Katalogleiste („Aus IFC-Datei übernehmen …", „Aus gbXML-Datei übernehmen …"), je mit eigenem Profil und der Regel „kein Delegat, kein Knopf" |
| Gebäudeexport (G7) | **offen, A17** | bei einem Schlüssel: `AppWurzel.razor` | offen | Empfehlung: **keiner**, aus demselben Grund — und der Einstieg zusätzlich dort, wo die Ergebnisse liegen (Bedarfsdialog) |
...
**A17 ist ein echter Widerspruch zwischen zwei geltenden Papieren**, und er ist beim Anwender: Das
...
mit Seitenschlüssel". Solange A17 offen ist, führt dieses Papier **beide** Wege mit ihren
Handgriffen: Der Menüweg brauchte zwei flache Zeilen unter *Daten & Import*
...
flowchart LR
  ...
  WIRT --> WF["GebaeudeWohnflaecheDialog"]
...
| `GebaeudeKatalogDialog` (Editor) | Überlagerung, vier Reiter | Bearbeiten, Neu, Verwaltung | **ein** Schreibweg (A4 = U1), Reiter 3 „Hülle und Rechenmodell", Reiter 4 „Zonen", der Knopf „Gebäude als eine Zone übernehmen" | G1 · G3 · G6 |
| `GebaeudeBedarfDialog` | Überlagerung | — | Raumtemperatur, Kühlbedarf, drei Spitzenkennzahlen, Vergleichstabelle beider Modelle, zweites Bild | G1 |
| `GebaeudeWohnflaecheDialog`, `GebaeudetypDialog` | Überlagerung | — | **unverändert** | — |
...
stateDiagram-v2
    ...
    state Editor {
        ...
        R3 : Reiter Huelle und Rechenmodell
        R4 : Reiter Zonen
        R1 --> R2
        R2 --> R3
        R3 --> R4
...
3. **Der Übergang Klassenweg → Bauteilweg wird benannt.** Das Anlegen der ersten Zone ändert die
   ...
   **Rückfrage vor der ersten Zone** („Ab jetzt rechnet dieses Gebäude über seine Bauteile statt über
   die U-Wert-Gruppen"), dazu eine `Herleitungszeile` am Modellschalter, die in **beiden** Stellungen
   sagt, was gilt. Ob die Datenlage der Träger bleibt oder ein dritter Persistenzwert entsteht, ist
   **A14**.
4. **Der Modellschalter schaltet Felder, er sperrt sie nicht.** Im Tagesbilanz-Weg werden die sieben
   Modellparameterfelder des Reiters 3 **unsichtbar** (U2, Empfehlung dort: verstecken; hier als
   **A19** geführt); die `Herleitungszeile` sagt in **beiden** Stellungen, was gilt. Gesperrte, aber
   sichtbare Felder wären nach der Hausregel begründungspflichtig — sieben Begründungen auf einmal
   erklären weniger, als sie verwirren. Der Feldbestandstest je Reiter (3.7) prüft **beide**
   Stellungen.
...
> er deshalb liegt. **Offen ist damit nur Name und Ordner (A3), nicht die Zahl der Dialoge.**
...
stateDiagram-v2
    ...
    Groesse : Groesse gegen Profil.MaxBytes — vor dem Lesen; bei .ifczip die ENTPACKTE Groesse aus dem Zip-Verzeichnis
...
| Kühlbedarf (Jahressumme, Monatswerte) | `GebaeudeBedarfDialog` als zweite Kennzahlgruppe; Monatsblatt des Ergebnisdialogs als zweite Reihe | `Kennzahlkachel`; Monatsbild als Stapel mit Heiz- und Kühlanteil. **Informative Reihe, kein vierter Kanal** |
...
| Vergleich Tagesbilanz ↔ Stundenmodell | `GebaeudeBedarfDialog`, Tabelle mit vier Spalten (Kennzahl · Tagesbilanz · VDI 6007 · Abweichung) über fünf Zeilen | **Zwei Auskünfte desselben Controllers**, keine zweite Rechnung (1.4); die Abweichung als `Kohaerenzzeile` |
...
bei Abbruch** sowie den Fall **ohne Gaben**.
...
| Stufe | Wiki-Arbeit |
|---|---|
| **G1 + G2** | neue Seite „Gebäudemodell VDI 6007"; die Seite „Gebäude" wird fortgeschrieben (Modellschalter, Reiter „Hülle und Rechenmodell", ein Schreibweg) |
...
**Zwei Aufrufer, zwei Verzweigungspunkte, ein Rechenweg.** `HeizwaermeEinesGebaeudes`
...
(`EPOS.Kern/Controller/GebaeudeBedarfCtrl.cs:117`) — und die Modellwahl fällt an **zwei Punkten**,
die **nicht** beide im Rumpf dieser Methode liegen. `HeizwaermeEinesGebaeudes` (`:566`) ruft im
Einheiten- und Bewohnerzweig **zuerst** `Bewohner_und_Flaeche_berechnen` (`:575`); dort, in der
Verbrauchsrückrechnung, steht der **erste** Verzweigungspunkt (`:647`, Methode `:613-656`). Der
**zweite** steht danach im Tagesmodellaufruf der Methode selbst (`:581`, Methodenende `:611`). Die
Rückrechnung läuft also **vor** der Modellwahl des Bedarfs, und die Modellwahl ist durch eine zweite
Methode **durchzureichen**. Beide verzweigen **gemeinsam**; bliebe `:647` beim Tagesmodell, während
der Bedarf aus dem Stundenmodell stammt, mischte die Verhältnisrechnung zwei Modelle — ADR-002,
Entscheidung Punkt 3 sagt es ausdrücklich: „Die Flächen- und Bewohnerrechnung, die vor der
Verzweigung läuft und selbst das Tagesmodell ruft, folgt derselben Modellwahl".

**Die Verzweigung umschließt den ganzen Tagesverteilungsblock, nicht nur den Aufruf.** Im
Tagesbilanz-Zweig bleibt alles Zeichen für Zeichen, wie es ist: Tageswerte rechnen (`:581`),
Tagesverteilung lesen (`:584`), **Abbruch bei fehlender Verteilung** (`:590-595`), Puffer nullen und
Stundenwerte verteilen. Im Stundenzweig entfällt dieser Block **vollständig** — das Stundenmodell
braucht keine Tagesverteilung und **erbt deshalb auch deren Abbruch nicht**. Es füllt denselben
Puffer, in **Watt**, und gibt `true` zurück. **Dass die Verzweigung hier den ganzen Block
umschließt, ist entschieden** — das Umsetzungskonzept 1.5 legt es fest. **A16** fragt allein, ob der
Punkt in `Bewohner_und_Flaeche_berechnen` (`:647`) denselben Zuschnitt bekommt und wie die
Modellwahl dorthin gelangt (Kap. 6).

**Alles davor und dahinter bleibt unberührt** — mit einer benannten Ausnahme: Die Ermittlung von
Bewohnerzahl und Wohnfläche vor `:647` bleibt, wie sie ist (der Eingangsbauer liest ihr Ergebnis
**danach**); die **Verbrauchsrückrechnung** in derselben Methode verzweigt dagegen mit. Unberührt
...
Tagesrechnung, nicht in einer Nachmultiplikation des Ergebnisvektors — **das Stundenmodell muss
diese Verhältnisrechnung selbst führen; sie fällt ihm nicht zu.** Der Fall „bisheriger Verbrauch
...
```mermaid
flowchart LR
  RUN["SimulationRunner.Simuliere"] --> WB["SimulationWaermebedarf<br/>Waermebedarf_berechnen"]
  ...
  LOOP --> VZ{"Gebaeude_Modell"}
  VZ -->|TAGESBILANZ| TAG["Tageswerte + Tagesverteilung<br/>Abbruch, wenn sie fehlt"]
  VZ -->|VDI6007 oder NULL| EIN["GebaeudeModellEingang.Bauen<br/>ruft GebaeudeKlimaweg"]
...
über `GebaeudeBedarfCtrl.Rechnen`, und dieser Weg ruft dieselben zwei Methoden wie die
Gebäudeschleife. Die eine Verzweigung trägt den Dialog damit **automatisch** mit; es gibt keinen
zweiten Rechenweg und keine zweite Prüfkette.
...
**Fünf neue Kennzahlen** mit stabilem Schlüssel in Kleinschreibung mit Punkt, wie der Bestand
...
| `gebaeude.kuehlbedarf` | Kühlbedarf (informativ) / cooling demand (informative) | MWh/a | `N0` | ja | **Summe** |
...
**Fünf Kennzahlen im Bericht, zwei weitere Größen nur im Export.** Die Anforderung nennt je Gebäude
**sieben** Größen; zwei davon bekommen **keinen** Katalogeintrag: „Stunden mit Kühlbedarf" (eine
Summe über Gebäude wäre sinnlos, ein Maximum wäre eine andere Aussage) und die **Gebäudespitze** (sie
steht schon als `Waermelast_Max` über den Kanalvektor im Bericht). Beide stehen als **Skalar je
Gebäude** im Referenzlauf-Export (4.4) — dort ist das Gebäude die Zeile, und die Frage der
Aggregation stellt sich nicht.
...
EPOS-Erweiterung, keine Norm, und die Datei darf das Gegenteil nicht nahelegen. Offen ist nur noch
die Frage nach Wiki-Seite und Berichtskopf (**A12**). Der Ausweis wechselt, sobald G0 den offenen
Testfall löst.
...
| **Der Rückweg-Test** | läuft auf einer **gitignorierten Arbeitskopie** der Testdatenbank, die alle Gebäude auf Tagesbilanz setzt, und hält jedes Referenzprojekt gegen die letzte reine Bestandsbasis; die eingefrorene Testdatenbank bleibt unberührt, der Schalter dafür ist ein eigener Modus des Referenzlaufs |
...
| Einstieg | Überlagerung im Gebäudedialog **und** im Bedarfsdialog (dort liegen die Ergebnisse) | **A17**; Empfehlung: kein Menüpunkt, kein Maskenschlüssel (D14) |
...
| Kennungen | deterministisch, aus dem Schlüsselpfad der **IDs**, nie aus Namen | Voraussetzung für jeden Modellvergleich beim Empfänger und für den Rundlauf; nachträglich nicht einzuführen (D5) |
...
keine Aufwandszahlen. Die stehen im Umsetzungskonzept Kap. 4, im Mehrzonenkonzept Kap. 9 und im
Datenaustauschkonzept Kap. 10.
...
| **G1 + G2** | `GebaeudeKlimaweg`, `GebaeudeModellEingang` samt `Bauen`, `GebaeudeModellErgebnis`, `Gebaeudepruefung`; die Verzweigung an beiden Punkten; der vierte Parameter der Auskunft; **ein Schreibweg im Editor** (A4), Reiter 3, Platzhalter am Zahlenfeld (A5), Modellschalter; **Hüllenumzug nach `EPOS.UI.Daten`** samt `Gebaeudewege` (A10); Bedarfsdialog mit Vergleich, zweitem Bild und drei Spitzenkennzahlen; Kennzahlen und Gruppe `GR_GEBAEUDE`; drei bedingte Reihen im Export; Glossarabschnitt, beide `.resx`, Dialogkatalog-Einträge; Wiki-Seite und Logbuch-Entwurf | M4 | **alle dreizehn Projekte ändern sich — Basis vollständig neu**; dazu der Rückweg-Test auf der Arbeitskopie; `ChartProben` grün; Sichtabnahme Windows; **Versionsnummer beim Anwender erfragt** |
...
| **G4c** (gbXML-Import) | `GbxmlLeser`, `GbxmlAbbild`, `GbxmlImportProfil`; zweites Profil am selben Dialog; Zonenregel für den Einzonenfall | **A7** (ADR-004), G1 + G2; G3 für Schichten | Importprobe bestanden; Referenzlauf unverändert. **Reihenfolge zu G4a ist ein Anwenderentscheid** (D1); **D16 ist vor der Beauftragung zu entscheiden** — er verschiebt die Zonenregeln nach G6c |
...
flowchart TB
  ...
  M3 --> M4["Merge M4 — Klimaspalten<br/>Regel Klimareihen"]
  M4 --> G12["G1 + G2 — Modell und Darstellung<br/>EINFRIEREN, Basis vollstaendig neu"]
  G12 --> G3["G3 — Bauteilkatalog<br/>SPERRPUNKT: Katalogliste + Rasterprobe"]
  G3 --> G4A["G4a — IFC-Import<br/>SPERRPUNKT: Lizenzhinweisseite"]
  G3 --> G4C["G4c — gbXML-Import<br/>SPERRPUNKT: A7 (ADR-004)"]
  G3 --> G6A["G6a — Zonen-Datenmodell"]
  G4A --> G6C
  G4C --> G6C
  G6A --> G6B["G6b — Zonenrechnung<br/>SPERRPUNKT: A8 (ADR-005)"]
...
Neunzehn Fragen. Vier davon (**A4**, **A5**, **A9**, **A19**) liegen bereits unter einer Nummer des
Umsetzungskonzepts beim Anwender und stehen hier **nur als Sperrpunkt dieses Papiers** — entschieden
werden sie unter ihrer alten Nummer, damit nicht zwei Register zwei Antworten bekommen.

| Nr. | Sachverhalt | Empfehlung | Alternative | Ohne Entscheid blockiert |
|---|---|---|---|---|
| **A1** | Bleibt die Kaskade der Gebäudekinder auf `Tab_Gebaeude`, obwohl der Gebäude-Schreibweg möglicherweise löscht und neu anlegt? Bei `Tab_Zonenluftstrom` greift die Falle **doppelt** (zwei Eltern) | **Kaskade behalten**, den Schreibweg **vor G3 messen** und die Rettung dort einbauen, wo das Löschen steht (`GebaeudekinderSichern` in `WizardCtrl`); die Probe prüft beide Fälle | (b) ohne Kaskade, Waisen über ein Werkzeug entfernen — verlagert die Verantwortung in ein Werkzeug, das beim Anwender nie läuft. (c) Schreibweg auf „Ändern statt Löschen" umbauen — sauber, aber ein Eingriff in den Bestandsweg mitten in der Einfrierkette | G3 (Anlage von `Tab_Zone`) |
| **A2** | Bleibt das IFC-Paket am Kern, oder zieht der Leser in ein eigenes Projekt hinter `IGebaeudeLeser`? | **Am Kern bleiben** (ADR-003), aber `IGebaeudeLeser` **von Anfang an** ziehen: Vermisst der iOS-Gerätebau Typen, kostet der Umzug eine Fabrikzeile statt eines Umbaus | (b) sofort eigenes Projekt — iOS verlöre den Import von vornherein. (c) ohne Naht binden — spart eine Schnittstelle, macht jeden Umzug zum Umbau | nichts; die Messung steht in G4 an |
| **A3** | **Name und Ordner** des Zuordnungsdialogs: `GebaeudeImportDialog` in `Dialoge/Import/` (dieses Papier) oder `IfcZuordnungDialog` in `Dialoge/Bedarf/` (Datenaustauschkonzept 2.4)? Dass es **einer** ist, ist dort entschieden | **`GebaeudeImportDialog`** — ein Format im Namen einer Maske, die zwei Formate trägt, ist eine Unwahrheit; der Dialog erbt den Ablauf des Katalogimports | (b) den Namen aus 2.4 behalten und die Zweiformatigkeit nur im Profil führen | G4 |
| **A4** | Ein Schreibweg im Katalogeditor statt der heutigen vier Aus- und Schreibwege — **für den Anwender sichtbar** | **= U1, Empfehlung dort: ja**, mit G1; „Speichern unter…" bleibt als nicht schließender Zweitknopf | siehe U1 | G1 (sonst hängen zehn Prüfregeln an drei Schreibstellen) |
| **A5** | Platzhalter am Zahlenfeld — ein Eingriff in einen Standardbaustein, den jeder Dialog benutzt | **= U3, Empfehlung dort: ja**, rein additiv; zieht `StilblattTests` nach sich | siehe U3 | G1 (Reiter 3 bekommt rund zehn Vorgabefelder) |
| **A6** | Werden Zonen, Bauteile und Luftströme als **ein** Aggregat je Gebäude geschrieben oder je Zone einzeln — und schreibt das Aggregat durch **Löschen + Neuanlegen** oder durch **Abgleich über die Ids**? | **Ein Aggregat** (`GebaeudeZonenCtrl`), und darin **Ändern statt Löschen**: Abgleich über die Ids des Arbeitsstands in der Reihenfolge Entfernen → Ändern → Anlegen, alles in **einer** Transaktion (1.4, 2.7). Nur so bleibt der eine Schreibweg über vier Überlagerungsebenen einer **und** bleiben die Ids stehen, an denen `Tab_Importzuordnung` (ohne Kaskade) und `Tab_Zonenluftstrom` (Kaskade an **beiden** Zonen, `CHECK ID_ZoneA < ID_ZoneB`) hängen | (b) Löschen + Neuanlegen je Gebäude — einfacher zu schreiben, zerstört aber bei **jedem** gewöhnlichen Speichern die Importherkunft und damit den Round-Trip. (c) je Zone schreiben — der Bauteildialog müsste selbst schreiben, und Abbrechen auf der Zonenebene ließe geschriebene Bauteile stehen | G6b |
| **A7** | Wird ADR-004 angenommen (gbXML über LINQ to XML mit handgeschriebenem Lesemodell)? | **= ADR-004, annehmen** — das aus dem Schema erzeugte Modell ist unbrauchbar, und ohne Entscheid hat G4c keinen Leseweg | (b) Serialisierer mit erzeugtem Modell — scheidet schon unter Windows aus und erzeugt Laufzeitcode, den iOS nicht trägt. (c) Fremdbibliothek — es gibt für .NET keine | **G4c vollständig** |
| **A8** | Wird ADR-005 angenommen (Zonenkopplung über die Nachbarraum-Randbedingung, Durchlauf je Stunde)? | **= ADR-005 / M1 und M4, annehmen — aber mit Messpflicht:** der einfachere Weg als Vergleichsrechnung, das Gesamtsystem für zwei Zonen als Prüforakel, die Probe „eine Zone bitgleich" als Gate | (b) Vorstundenkopplung — einfacher und schneller, trägt aber den Zonen-Luftaustausch nicht. (c) Gesamtsystem — exakt, aber teuer genau dort, wo das Regelungsmuster je Stunde festgehalten wird | **G6b vollständig** |
| **A9** | Die **zwei Gebäudespalten-Schritte** des Konzepts zu **einem** verschmelzen (Papiername **M3**)? Die Zahlen, die das Konzept dafür nennt, sind anderweitig vergeben (2.4) | **= U5, Empfehlung dort: ja** — 15 Spalten je Tabelle, ein Sichtneubau; der Klimaschritt bleibt getrennt | siehe U5 | M3 |
| **A10** | Zieht der Gebäudedialog schon mit G1 nach `EPOS.UI.Daten` oder erst mit G6? | **Mit G1** — die Hülle wird ohnehin neu geschnitten, und der Importweg setzt einen plattformfreien Schreibweg voraus | (b) erst mit G6 — spart im ersten Schritt Arbeit, verdoppelt sie aber, und der Import bleibt auf iOS bis dahin benannt abgelehnt | G4 (plattformfreie Importhülle) |
...
| **A14** | Was trägt den Umschalter **Klassenweg → Bauteilweg**? Heute die Datenlage: `Tab_Zone` leer heißt Klassenweg (Mehrzonenkonzept 4.3). Damit ändert das Anlegen der **ersten** Zone die Zahlen eines Projekts ohne Schalter | **Datenlage behalten, aber den Übergang benennen**: Rückfrage vor der ersten Zone, `Herleitungszeile` am Modellschalter in **beiden** Stellungen, und der Übergang ist ein Einfrieranlass nur dort, wo ein Referenzprojekt Zonen bekommt (G6d) | (b) dritter Persistenzwert an `Gebaeude_Modell` — der Weg steht dann ausdrücklich in der Datenbank, kostet aber einen Wert, der beim Löschen der letzten Zone wieder falsch wäre | G3 (der Übernahmeknopf entsteht dort) |
| **A15** | Was geschieht mit der letzten reinen Bestandsbasis (GB), gegen die der Rückweg-Nachweis läuft? Befund V 5.2 will sie aufheben; die Hausregel sagt „frühere Basen liegen nicht mehr im Repositorium, gerechnet wird ausschließlich gegen die aktuelle Basis" | **Keine zweite Basis im Repositorium, sondern ein Referenzprojekt, das dauerhaft auf Tagesbilanz steht und in der neuen Basis mit eingefroren wird** (so auch die Empfehlung des Systementwurfs 8.4). Dann prüft jeder Lauf **beide** Wege gegen dieselbe, aktuelle Basis, und die drei neuen Reihen entstehen für dieses Projekt gar nicht erst — genau die Bedingung, die der Vergleich ohnehin stellt. Die GB-Basis bleibt nur bis zum Merge G1 + G2 und wandert dann mit ihrem Protokoll nach `Dokumentation/ueberholt/Referenzbasen/` | (b) zwei Basen dauerhaft — gegen die Hausregel, und jeder Lauf müsste sagen, gegen welche er misst. (c) ein Dateiausschluss im Vergleich — löste es technisch, kostet aber einen neuen Schalter an einem Werkzeug, an dem die ganze Nachweiskette hängt | G1 + G2 |
| **A16** | **Welchen Zuschnitt bekommt der Verzweigungspunkt in `Bewohner_und_Flaeche_berechnen`?** Für den Punkt im Rumpf (`:581`) ist es entschieden: Umsetzungskonzept 1.5 legt fest, dass im Stundenzweig der ganze Tagesverteilungsblock entfällt — samt dem Abbruch bei fehlender Verteilung (`SimulationWaermebedarf.cs:590-595`), den das Stundenmodell nicht erbt. **Neu offen ist allein**, ob der Punkt in `Bewohner_und_Flaeche_berechnen` (`:647`) denselben Zuschnitt bekommt, und wie die Modellwahl dorthin gelangt — die Methode kennt sie heute nicht (4.1) | **Derselbe Zuschnitt, durchgereicht**: Die Modellwahl wird `Bewohner_und_Flaeche_berechnen` als Parameter mitgegeben, und im Stundenzweig entfällt auch dort der Tagesverteilungsblock. Der Tagesbilanz-Weg bleibt an **beiden** Stellen Zeichen für Zeichen, wie er ist | (b) die Rückrechnung beim Tagesmodell lassen — dann mischt die Verhältnisrechnung zwei Modelle, was ADR-002 Punkt 3 ausschließt. (c) die Modellwahl über ein Feld der Instanz reichen statt über den Parameter — spart die Signatur, stellt aber einen Zustand her, der zwischen Gebäuden reisen kann (1.6) | G1 (die Verzweigung selbst) |
...
| **A19** | Werden die sieben Modellparameterfelder des Reiters 3 im Tagesbilanz-Weg **versteckt** oder **gesperrt gezeigt**? Das ist ein Zustand der Maske (3.2) und für den Anwender sichtbar | **= U2, Empfehlung dort: verstecken** — die `Herleitungszeile` sagt in beiden Stellungen, was gilt; sieben gesperrte, aber sichtbare Felder wären nach der Hausregel („gesperrt heißt sichtbar, und wer seinen Grund erklären soll, ist `aria-disabled`") sieben Begründungen auf einmal | siehe U2 | G1 (Reiter 3 und sein bunit-Feldbestandstest) |
...
Anwenderentscheide des Datenaustauschkonzepts und stehen dort. Ebenso bleiben **U1–U16**, **M1–M14**
und **D1 ff.** dort offen, wo die Konzeptpapiere sie führen.
...
- **Aufwände und Stufeninhalte.** Personentage, Reihenfolge innerhalb einer Stufe und der Umfang je
  ...
  Kap. 4, im [Mehrzonenkonzept](Konzept_Mehrzonenmodell_IFC_EPOS-Plan.md) Kap. 9 und im
  [Datenaustauschkonzept](Konzept_Datenaustausch_gbXML_IFC_EPOS-Plan.md) Kap. 10. Kapitel 5 nennt
...
- **Alles, was das Konzept in Kapitel 15 ausschließt:** Feuchtebilanz, Kühlung als vierter Kanal,
  Bauteilaktivierung, Kopplung von Vorlauftemperatur und Wärmepumpen-Fahrplan an die Raumtemperatur,
...
- **Entscheide.** Dieses Papier entscheidet nichts, was dem Anwender zusteht: **A1 bis A19** sind
  Fragen mit Empfehlung, ADR-004 und ADR-005 bleiben bis zu seinem Wort Vorschläge.
