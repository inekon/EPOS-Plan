...
namespace WindowsFormsApplication1
{
    ...
    public static class WPDataCtrl
    {
        ...
        public static List<WPData> ReadAll()
        {
            ...
            ctrl.ReadAll();   // alle Stamm-WP (Tab_WP_STAMM), sortiert nach Bezeichner

            // Min/Max Vorlauf je Stamm-WP aus den Kennlinien-Stammdaten ermitteln
            System.Collections.Generic.Dictionary<int, int> minD = new System.Collections.Generic.Dictionary<int, int>();
            System.Collections.Generic.Dictionary<int, int> maxD = new System.Collections.Generic.Dictionary<int, int>();
            System.Data.DataTable dtv = DataRepository.GetDataTable(
                "SELECT ID_WP, Min(Vorlauf) AS MinV, Max(Vorlauf) AS MaxV FROM Tab_Kenndaten_STAMM GROUP BY ID_WP");
            if (dtv != null)
            {
                foreach (System.Data.DataRow r in dtv.Rows)
                {
                    if (r["ID_WP"] == System.DBNull.Value) continue;
                    int wpid = System.Convert.ToInt32(r["ID_WP"]);
                    minD[wpid] = r["MinV"] != System.DBNull.Value ? System.Convert.ToInt32(r["MinV"]) : 0;
                    maxD[wpid] = r["MaxV"] != System.DBNull.Value ? System.Convert.ToInt32(r["MaxV"]) : 0;
                }
            }
            ...
            for (int i = 0; i < ctrl.rows; i++)
            {
                dat = new WPData
                {
                    ...
                    MaxVorlaufTemp = maxD.ContainsKey(ctrl.items[i].ID) ? maxD[ctrl.items[i].ID] : 0,
                    MinVorlaufTemp = minD.ContainsKey(ctrl.items[i].ID) ? minD[ctrl.items[i].ID] : 0,
