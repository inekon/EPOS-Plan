    ...
    class StromverbraucherStammCtrl : StromverbraucherModel
    {
            ...
            using (DbVorgang v = DataRepository.Vorgang())
            {
                try
                {
                    ...
                    for (int i = 1; i <= 12; i++) { cols.Append(", Monat_" + i); vals.Append(", ?"); }
                    {
                        ...
                        p.Add(new DbParam("@hid", neuSvId));
                        p.Add(new DbParam("@hproj", idProjekt));
                        p.Add(new DbParam("@hbez", szBezeichner));
                        p.Add(new DbParam("@htyp", (object)typName ?? DBNull.Value));
                        p.Add(new DbParam("@hbesch", ColOrNull(h, "Beschreibung")));
                        for (int i = 1; i <= 12; i++)
                            p.Add(new DbParam("@mon" + i.ToString("D2"), ColOrNull(h, "Monat_" + i)));
        ...
        public static int TypNew(string typname)
        {
            ...
            bool ok = DataRepository.ExecuteSQL(
                ...
                new DbParam("@ttypn", DbParamTyp.VarWChar) { Wert = (object)(typname ?? "") },
