> Archivierte Fassung der `WindowsFormsApplication1/CLAUDE.md` vom 12.09.2026, vor der Kürzung auf den gültigen Stand. Die relativen Verweise sind auf diesen Ablageort umgeschrieben; Zählungen, Daten und Kürzel beschreiben den Stand von damals.
...
- **`Controller/`** (**2** Dateien, `*Ctrl.cs`; `ErststartCtrl` ist mit W3 vom
  09.09.2026 gefallen) — was Oberfläche braucht: `MenueCtrl`
  ...
  und `EnergietraegerKatalogCtrl`. **Die zwölf `*KontextMenuCtrl` sind
...
- **`Views/`** (**98 `.cs`**; **112 Dateien** mit `.resx`) —
  ...
  **Mit iU9‑W16c ist die MISCHPHASE ZU ENDE — das Hauptfenster ist Razor.**
  ...
  der Besitzer für jede `BlazorDialogForm`, `ProcessCmdKey` + F1 (bis #228
  `KeyPreview` + `KeyDown` — das sieht keine Taste, die in der WebView2
  anfällt, Befund KI‑D‑B‑3), der Ladeanstoß
...
- **`Allgemein/`** (**38** `.cs`; 40 vor iU9‑W14c — `GrafikTools/ChartManager.cs` (560 Z. samt
  ...
  (`Update/SchemaMigration.cs`, der Oberflächenbaustein `HelpExtender` in
  `Hilfe/HelpCatalog.cs`) sind im iU5-Statusblock des Umsetzungskonzepts begründet.
  **`Update/ErststartMigration.cs` ist mit W3 (#157‑E‑1, 09.09.2026) gelöscht** — der
  Übernahme-Assistent aus Access ist gefallen; was bleibt, ist das Hauswerkzeug
  `EposSqliteMigrator` und der eingefrorene Access-Zweig
  `SchemaMigration.HebeAltbestand`. Die Datenbank einer Neuinstallation entsteht
  seither im KERN (`EPOS.Kern/Allgemein/Datenbank/Erstbereitstellung.cs`), gerufen
  aus `Program.DatenbankBereitstellen()`. `Simulation/`, `Wirtschaftlichkeit/`, `Bericht/`
...
| `Blazor/` | **Die Hülle für Razor-Dialoge und -Seiten (iU8 / iU9-W5).** `BlazorDialogForm<T>` — ein modales `Form` mit `BlazorWebView`, das eine Komponente aus `EPOS.UI` zeigt und ihr Ergebnis als `DialogResult` zurückgibt; `DpiInsel` (P/Invoke `SetThreadDpiAwarenessContext`); `BlazorDienste` — das Dienstverzeichnis der WebView, einmal gebaut; seit iU9-W1.2 `NamensDialogHuelle` für die fünf zeichengleichen Namensabfragen des Bestands (seit iU9-W2.1 alle fünf umgestellt: `Bezeichner`, `BezeichnerUndBeschreibung`, `FragenMitHinweis`); seit iU9-W2.2 `Sprungbruecke` — Schlüssel → `Form`, **modal aus dem Rückruf einer Razor-Komponente heraus** (nur WinForms-Ziele; seit iU9-W6.0d auch die vier Katalogverwaltungen der Erzeuger, seit iU9-W7.0f die Stammdaten der Solarthermieganglinien, seit iU9-W10a.0c die Pufferspeicher-Verwaltung NUR ZUM ANSEHEN — `PufferSpAdminNurLesen`, ein eigener Schlüssel, weil derselbe Sprung ohne das Kennzeichen aus dem Nachschlagen das Bearbeiten des Auslieferungskatalogs machte); seit iU9-W6.0e `BlazorAssistentSeite<T>` — dasselbe für eine ASSISTENTENSEITE: randlos, `TopLevel = false`-tauglich, die WebView verzögert in `Bestuecken` gebaut (Risiko R5), beim Wiederbesuch wird die Wurzelkomponente getauscht statt der WebView. Seit iU9-W4.0 gilt für Blazor-Ziele nicht mehr der nachgelagerte Sprung, sondern der Baustein `Ueberlagerung`: ein modaler Bereich IM selben Fenster, also ohne zweite WebView (Risiko R2). Die Hülle liefert dafür `Gaben()` statt `Oeffnen()`. **Seit iU9-W5.0 gibt es die zweite Hüllenform: `BlazorSeite<T> : UserControl`** — nicht-modal, für eine Seite, die in einer vorhandenen Maske sitzt und dort bleibt (`Form_Start.tabPage6`). Sie trägt dieselben `CreationProperties` wie die Dialoghülle, insbesondere denselben `UserDataFolder`: ein gemeinsamer Browserprozess für Dialoge und Seiten. **Eine WebView je Fenster** (Risiko R5) — umgeschaltet wird in der Komponente (Baustein `Reiter` bzw. die Navigation von `BerichteKostenSeite`), nicht durch eine zweite Hülle. Der Projektwechsel läuft über `EPOS.UI.Dienste.SeitenZustand`, ein Objekt mit Änderungsereignis, damit die WebView **nicht** neu gebaut wird. **Regel seit Befund W16c‑B12 (04.09.2026): Jede Komponente, die in einer `BlazorSeite<T>` sitzt, MUSS einen `[Parameter] SeitenZustand? Zustand` führen** — die Hülle trägt ihn jedem Parametersatz nach, und ein Schlüssel ohne `[Parameter]` bricht beim ERSTEN Zeichnen (unter Windows sichtbar als `TargetInvocationException` an `Application.Run`). Der Konstruktor prüft das seither per Reflexion und meldet den Typnamen; die bunit-Fälle `HauptfensterTests.AusHuelle` rendern denselben Weg. **DPI:** Die `DpiInsel` wirkt nur für den modalen Lauf; eine eingebettete Seite sitzt im Fenster der DpiUnaware-`Form_Start` und wird ab 125 % bitmapskaliert — `BlazorSeite` versucht es deshalb gar nicht erst und dokumentiert den Befund (offener Entscheid iF21). **Drei Regeln seit Befund W16b‑B‑1 (Windows-Abnahme 05.09.2026, „die Dialoge auf der Startseite sind leer"):** (a) **Jede Hülle meldet WebView-Fehler** — der WinForms-`BlazorWebView` (10.0.100) führt **kein** `UnhandledException`-Ereignis (nur `UrlLoading`, `BlazorWebViewInitializing`, `BlazorWebViewInitialized`), eine gescheiterte WebView2-Initialisierung bleibt also von sich aus still; `WebViewWache` hängt sich an `CoreWebView2InitializationCompleted` und `BlazorWebViewInitialized` und legt eine **Frist von zehn Sekunden** darüber — danach steht statt der beigen Fläche ein markierbarer Text mit Typ, Wortlaut und innerster Ausnahme, derselbe Satz geht nach `Debug`/`Trace`. Wer eine dritte Hüllenform baut, hängt sie an. (b) **Die Parametersatz-Wache pflegen** — `Parametersatzwache` prüft beim Bauen JEDEN Schlüssel des Wörterbuchs gegen die `[Parameter]` der Komponente (die Verallgemeinerung von W16c‑B12) und nennt den fehlenden im Klartext; die Gegenwache auf Linux ist `EPOS.UI.Tests/ParametersatzTests`, die den Quelltext aller `Views/**/*Huelle.cs` liest (61 Stellen). Eine Komponente mit `CaptureUnmatchedValues` ist ausgenommen. (c) **Kein `ShowDialog` unmittelbar aus einem Blazor-Ereignis** — seit Startseite und Hauptfenster Razor sind, kommt jeder Kachelklick und jeder Menüpunkt aus dem `WebMessageReceived`-Rückruf der ersten WebView2, und ein modales Fenster mit einer ZWEITEN WebView2 entstünde INNERHALB dieses Rückrufs (genau das, was `Sprungbruecke` als Risiko R2 ausschließt). `Blazorsprung.Verzoegert` lässt das Ereignis zu Ende laufen; benutzt wird er in den zwei Verteilern `StartseiteHuelle.Kachelweg` und `HauptfensterHuelle.Weg`, nicht in den 21 bzw. 55 Aufrufern. Die Zuständigkeit („behandle ich das?") bleibt dabei synchron und kommt aus DERSELBEN Fallunterscheidung, die den Ablauf liefert. **Seit Befund KI‑D‑B‑1 (Anwender, 11.09.2026, Auftrag #219) gibt es einen DRITTEN Verteiler: `WinFormsNavigation`, Fall `Masken.KiAssistent`.** Die Regel galt bis dahin für Kacheln und Menüpunkte — sie gilt für JEDEN Weg, der aus einem Blazor-Ereignis ein Fenster mit einer zweiten WebView2 aufmacht, und der Hilfe-Assistent kommt seit #199 auch aus der Hilfe-Pille JEDER Maske (`InfoKnopf` → `KiAssistentWeg.AusDialog` → `Dienste.Navigation.OeffneMaske`). Dort stand der blanke Aufruf, während derselbe Assistent über das Menü längst verzögert aufging — der Anwender konnte in dem Fenster nicht tippen. **Ein nicht-modales Fenster braucht zusätzlich die TASTATUR:** `ShowDialog` bringt seine eigene Nachrichtenschleife mit und holt die Eingabe von selbst, ein `Show(besitzer)` tut das nicht, und ein `Activate()` stellt das Fenster nur vor die anderen. `BlazorDialogForm.TastaturUebergeben()` ist dafür die eine Stelle — `Activate()`, dann Fokus auf die innere WebView2, zweimal (sofort und nach `CoreWebView2InitializationCompleted`, weil sie sich asynchron aufbaut); die DOM-Seite gehört der Komponente (`ElementReference.FocusAsync` auf ihr Eingabefeld). Wachen auf Linux: `EPOS.UI.Tests/KiChatOeffnerTests`. **Zwei Regeln kommen mit Befund KI‑D‑B‑3 (Anwender, 11.09.2026, Auftrag #228, „der Hilfe-Assistent lässt sich nicht mehr aus dem Hauptmenü aufrufen, auch mit F1 nicht") dazu:** (f) **Der Riegel von `Blazorsprung` fällt VOR dem Sprung, nicht danach** — er hieß seit W16b im Klassenkopf „gilt nur bis zum **Beginn** des Sprungs", stand aber im `finally` von `Ausfuehren` und damit bis zu dessen Ende. Solange jeder Sprung eine Ebene tief war, fiel das nicht auf; seit #219 ist der Menüweg zum Assistenten ZWEI Ebenen tief (`HauptfensterHuelle.Weg` erkennt `"KI_ASSISTENT"` in `Maskenschluessel` und verzögert, im geposteten Sprung verzögert `WinFormsNavigation` noch einmal), und der innere Ruf traf den Riegel des äußeren und kehrte stumm zurück. **Wer einen Verteiler baut, der aus einem verzögerten Sprung heraus einen zweiten anstößt, verlässt sich auf diese Reihenfolge.** Dazu: ein abgewiesener Sprung steht im Protokoll (`[Blazorsprung]` nach `Debug`/`Trace`), ein **verwaister** Riegel verfällt nach fünf Sekunden (eine mit `BeginInvoke` eingereihte Nachricht läuft nie, wenn ihr Wirtsfenster vorher abgebaut wird — ohne Frist wären Menü, Kacheln und Pillen auf einen Schlag stumm), und `Blazorsprung.Wirtsfenster` fällt auf `Program.rahmen` zurück, wenn `Form.ActiveForm` null ist — EINE Ermittlung für Nachrichtenschlange **und** Fensterbesitzer. (g) **Eine Taste des Hauptfensters fängt `ProcessCmdKey`, nicht `KeyPreview`** — `KeyPreview` wirkt über `Control.ProcessKeyMessage` und damit nur für Tasten, die im `WndProc` eines WinForms-Steuerelements ankommen; das Browserfenster der WebView2 ist keines, und seit W16c sitzt der Tastaturzeiger praktisch immer dort. `ProcessCmdKey` erreicht der Tastendruck über `PreTranslateMessage` → `Control.FromChildHandle` → `PreProcessMessage` und die Elternkette hinauf; der Weg deckt den bisherigen mit ab. **Und eine ADRESSE ist keine Datei** (Befund KI‑D‑B‑2, derselbe Tag): `Dienste.Datei.MitSystemOeffnen` prüft `File.Exists` und liefert für `https://…` immer `false` — still und ohne Meldung; der Weg heißt `Dienste.Datei.AdresseOeffnen` (so steht es seit iU9‑W16c.3 in `IDateiDienst` selbst). Derselbe Quelltextzeuge hält das fest. **Regel seit dem Anwenderwunsch 05.09.2026 (iU8‑E‑1, „Admin-Menüs sind nicht an Größe Bildschirm angepasst"): Ein FACHDIALOG öffnet im ANTEIL DES ARBEITSBEREICHS** — das Vorgabemaß ist das Maximum aus dem Wunschmaß der Hülle (`MASS`) und 85 % Breite × 90 % Höhe des Bildschirms, auf dem das Besitzerfenster steht, gedeckelt auf 92 % (der Deckel vom 03.09.2026 bleibt und hilft weiter nach oben). Bis dahin galt nur der Deckel, und ein Katalogdialog mit `MASS = 760 × 640` blieb auch auf einem 1920er Schirm 760 × 640 groß. **Die Rechnung steht plattformfrei in `EPOS.UI/Dienste/Fenstermass.cs`** — sie ist Arithmetik auf vier ganzen Zahlen und deshalb auf Linux prüfbar (`EPOS.UI.Tests/FenstermassTests`); die Hülle besorgt nur den Arbeitsbereich. Dort stehen seither auch die zwei Kleinstmaße (520 × 360). Beide Maße sind **Gerätepixel** desselben Schirms — unter „Per Monitor V2" (E‑6 / iF21) stehen `Screen.WorkingArea` und `Form.ClientSize` im selben Raum, deshalb kein Skalierungsfaktor. **`Dialogart.Klein` wächst NICHT mit** und ist an genau vier Hüllen gesetzt (`NamensDialogHuelle`, `KiEinstellungenHuelle`, `KiHinweisHuelle`, `LizenzHuelle`; `ErststartHuelle` ist mit W3 gefallen) — ein Fenster mit vier Feldern über den halben Bildschirm zu ziehen macht es nur leerer; alles andere ist `Dialogart.Fachdialog`, die Vorgabe. Die andere Hälfte der Sache steht in `../EPOS.UI/CLAUDE.md`: **Katalogdialoge nutzen die gewonnene Höhe** (Baustein `Katalograhmen`). **Zwei Regeln kommen mit Befund W13‑B‑1 (Windows-Abnahme 05.09.2026, „VDI-3805-Import: Absturz bei Datei laden, teilweise Absturz auch bei Dateiauswahl-Dialog") dazu:** (d) **Kein modales SYSTEMFENSTER im WebView-Rückruf** — Regel (c) hieß bis dahin „kein `ShowDialog`" und meinte eine zweite WebView2; sie reicht weiter. Ein `OpenFileDialog` und eine `MessageBox` sind ebenfalls modale Fenster mit einer verschachtelten Nachrichtenschleife, und im `WebMessageReceived`-Rückruf richten sie denselben Schaden an — nur in der WebView, die gerade zeichnet: Während die Schleife pumpt, liefert die WebView2 weitere Nachrichten aus, und Blazor beginnt einen Zeichenlauf, während einer läuft. Ob das gutgeht, hängt an der Zeitlage — daher das „teilweise" der Meldung. **Ein Dienst, der ein modales Fenster hochfährt, fährt es HINTER dem Ereignis hoch:** `IDateiDienst` und `IDialogDienst` führen dafür wartbare Zwillinge mit Standardfassung (`DateiOeffnenAsync`, `DateiSpeichernAsync`, `OrdnerWaehlenAsync`, `MeldungAsync`, `WarnungAsync`, `FrageAsync`), die Windows-Fassungen posten sie über `Allgemein/Blazor/Blazornachlauf.cs` eine Nachricht später. `Blazornachlauf` ist der Bruder von `Blazorsprung` für den Fall MIT Rückgabewert — der Sprung liefert nichts und kommt mit `BeginInvoke` aus, ein Dateiwähler MUSS einen Pfad liefern und gibt deshalb einen `Task`. **Wer aus einer Razor-Komponente heraus wählt oder meldet, ruft die `…Async`-Form und `await`et sie**; die synchronen Formen bleiben für die Aufrufer, die nicht warten können (`FileDlgClass`, `CsvExportClass` im Kern) und nicht aus einem Blazor-Ereignis kommen. **Auf einen `…Async` NIE blockierend warten** (`.Result`, `.Wait()`, `GetAwaiter().GetResult()`) — der Nachlauf braucht genau den Faden, den man damit anhält. In der Maske selbst gilt weiterhin zuerst A‑8 aus W11b: Warnbanner statt Meldung, Rückfrage-Baustein statt `MessageBox`. (e) **Jede Wurzel steht in der FEHLERSCHRANKE** — eine Ausnahme aus einem Ereignis oder aus dem Lebenszyklus einer Komponente ging bis W13‑B‑1 ungefangen an den Renderer und beendete den PROZESS (dieselbe Lücke, die `WebViewWache` im Klassenkopf als ihre Grenze nennt). Die drei Hüllen mounten deshalb `EPOS.UI.Bausteine.Wurzel<T>` statt `T` — eine `ErrorBoundary` mit `T` darin, denn eine Boundary fängt ihre NACHFAHREN und eine Wurzelkomponente hat nichts über sich. Der Parametersatz geht unverändert durch (`CaptureUnmatchedValues`), die `Parametersatzwache` prüft weiter gegen `T`. Wer eine vierte Hüllenform baut, mountet ebenfalls `Wurzel<T>`. Die **einzige** Stelle, an der WinForms und Blazor aufeinandertreffen |
...
(`DbParamOleDb`, `SchemaVersionAccess`, `SchemaMigration`, `GeraeteWaisen`) — und der ist seit
W3 (09.09.2026) ein Hauswerkzeug: `ErststartMigration` ist gelöscht, die Anwendung übernimmt
keinen `.accdb`-Bestand mehr;
...
`Dienste.Lizenzablage`) · `System.Data.OleDb` (**nur** noch für den eingefrorenen
Access-Zweig der Schemapflege, `SchemaMigration.HebeAltbestand` — ein Hauswerkzeug seit
W3) · `Mscc.GenerativeAI` · seit iU8
