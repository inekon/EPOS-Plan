    ...
    public class ProjektDuplizierenCtrl
    {
        ...
        public int Duplizieren(string quelleName, string neuerName)
        {
            ...
            try
            {
                ...
                List<Spec> specs = ErmittlePlan(conn);
                ...
                foreach (Spec s in specs)
                {
                    if (!offset.ContainsKey(s.Tabelle)) continue; // keine Quellzeilen
        ...
        private List<Spec> ErmittlePlan(OleDbConnection conn)
        {
            // Zuerst die deklarierten Beziehungen einlesen (fuer Reihenfolge + Offset-Ziel).
            _echteFks = LiesEchteFks(conn);
            System.Diagnostics.Debug.WriteLine("echteFks=" + _echteFks.Count);
            var alle = new List<Spec>();

            // Tab_Projekt als Wurzel (Name wird ersetzt).
            var projekt = new Spec { Tabelle = "Tab_Projekt", Pk = "ID", Filter = "ID = {0}", NameSpalte = "Projektname", Cols = Spalten("Tab_Projekt") };
            alle.Add(projekt);
            ...
            foreach (DataRow row in tabs.Rows)
            {
                ...
                {
                    alle.Add(new Spec { Tabelle = name, Pk = ErmittlePk(cols), Filter = KINDER[name], Cols = cols });
                }
                else if (Enthaelt(cols, "ID_Projekt"))
                {
                    alle.Add(new Spec { Tabelle = name, Pk = ErmittlePk(cols), Filter = "[ID_Projekt] = {0}", Cols = cols });
                }
                else if (Enthaelt(cols, "ProjektID"))
                {
                    alle.Add(new Spec { Tabelle = name, Pk = ErmittlePk(cols), Filter = "[ProjektID] = {0}", Cols = cols });
                }
                // sonst: keine Projekt-Tabelle -> ignorieren
            }
        ...
        /// </summary>
        private Dictionary<string, string> LiesEchteFks(OleDbConnection conn)
        {
            var map = new Dictionary<string, string>(StringComparer.OrdinalIgnoreCase);
            try
            {
                DataTable fk = conn.GetOleDbSchemaTable(OleDbSchemaGuid.Foreign_Keys, null);
                if (fk == null) return map;
            ...
            catch { /* Provider unterstuetzt das Rowset nicht -> Fallback auf FK_MAP/FK_OVERRIDE */ }
            return map;
        }
