...
iF1–iF9 stammen aus [`Konzept_iOS-Portierung_EPOS-Plan.md`](../ueberholt/Konzept_iOS-Portierung_EPOS-Plan.md)
...
| **iF20** | **Verteilung der WebView2-Laufzeit.** Online-Bootstrapper (~2 MB, braucht beim Setup Internet), Standalone-Installer (~150 MB, offline) oder Fixed-Version-Verteilung (Laufzeit im Programmordner, Aktualisierung liegt dann bei uns)? | Bootstrapper, solange kein Kunde ohne Internet installiert; sonst den Standalone-Installer beilegen | vor der Auslieferung von iU8 | **entschieden 03.09.2026: Bootstrapper.** Umgesetzt ist der **Bootstrapper** (iU8-10, `eafbc1f`): das Setup nimmt ihn nur mit, wenn die Laufzeit fehlt, und läuft weiter, wenn er scheitert. Der Standalone-Installer kommt erst dazu, wenn ein Kunde ohne Internet installiert (S10 im Setup-Konzept § 5.5 damit geschlossen) | Bootstrapper | 03.09.2026 |
| **iF21** | **DPI.** Die Anwendung ist bewusst `DpiUnaware`; die WebView2 wäre darin bei 125–200 % bitmapskaliert und sichtbar unscharf. Bleibt die Hülle eine **DPI-Insel** (`PER_MONITOR_AWARE_V2` nur für die Dauer des modalen Laufs) — oder wird die Anwendung als Ganzes DPI-fähig? | Insel; die Umstellung der 120 Masken ist ein eigenes Vorhaben | iU8, spätestens iU9 | **entschieden 03.09.2026, wie empfohlen:** die Insel bleibt für alle modalen Dialoge; die Anwendung wird **mit Welle 16** insgesamt DPI-fähig (Manifest und Programmstart auf Per-Monitor-V2), wenn nur noch der Rahmen WinForms ist — bis dahin sind die eingebetteten Seiten (W5 Berichte, W6 Assistentenseiten, später Simulationsreiter) bei 125–200 % bitmapskaliert (W5‑O1). Gebaut ist die `DpiInsel` (`Allgemein/Blazor/BlazorDialogForm.cs`, iU8-6). **Windows-Befund offen:** 125 % und 150 % sind noch nicht am Gerät gesehen worden; auf Windows vor 10/1803 greift die Insel nicht. Prüfpunkte in `Umsetzung_iU8_Nachweise.md` | Empfehlung: Insel jetzt, Anwendung DPI-fähig mit W16 | 03.09.2026 |
| **iF22** | **Präzisierung zu iF16 — wie viele Chart-Stacks trägt das Haus?** | **eine Bibliothek (SkiaSharp), zwei Nutzungsarten** | iU7/iU8 | **Bericht und Blazor** bekommen ein **Bild aus dem Kern-Renderer** (`EPOS.Kern/Allgemein/Bericht/ChartRenderer.cs`, seit iU7-2 SkiaSharp statt GDI+); die **interaktiven Bildschirmmasken** bleiben bei **ScottPlot** — und das ist heute genau **eine** Maske, `Form_SpeicherOptimierung`. ScottPlot 5 rendert selbst über SkiaSharp: es bleibt bei **einer** Grafikbibliothek, nur bei zwei Nutzungsarten. iF16 ist damit präzisiert, nicht ersetzt | bestätigt, wie vorgeschlagen | 03.09.2026 |
| **iF23** | **Was geschieht mit `ChartRendererGdi.cs`?** Der wortgleiche GDI+-Stand aus iU7-1 ist der Gegenpart des Windows-Bildvergleichs — eine zweite, nicht gepflegte Fassung desselben Renderers | **ersatzlos löschen**, sobald der Bildvergleich abgenommen ist | nach der Windows-Abnahme von iU7 | **ausgeführt 03.09.2026** auf Anweisung des Anwenders („führe aus iF23 bis iF25“): `ChartRendererGdi.cs`, `Referenzlauf/Bildvergleich.cs` und der Modus `bildvergleich` sind gelöscht — **ohne** dass der Windows-Bildvergleich je gelaufen ist. Die Berichtskette hat keine GDI+-Stelle mehr; der Pixelvergleich alt/neu ist damit nicht mehr möglich. Was bleibt: die Renderer-Tests im Kern, `ChartProben` (Maße, Farben, Determinismus) und der Sichtvergleich der Berichte am Gerät (Abnahmepunkt in `Umsetzung_iU9_Nachweise.md`) | löschen; ausgeführt ohne vorherigen Bildvergleich | 03.09.2026 |
| **iF24** | **Apple-Developer-Konto** (99 €/Jahr) jetzt beschaffen — oder erst nach einem grünen Simulator-Lauf? | erst **nach** iU10-6; bis dahin kostet der Nachweis nur CI-Minuten. Der Simulator-Job braucht weder Konto noch Zertifikat. Gerätebau und TestFlight sind iU13 | vor iU13 | **entschieden: beschaffen.** Die Schritte des Anwenders (Apple ID, Enrollment als Organisation mit D-U-N-S-Nummer, Bundle-ID `de.inekon.eposplan`, Zertifikat und Profil, CI-Geheimnisse) stehen in `Umsetzung_iU10_Nachweise.md`, Abschnitt iU13 | beschaffen (jetzt) | 03.09.2026 |
| **iF25** | **Seed-Datenbank im App-Bundle.** E1 = Testdatenbank (77 MB, nur CI), E2 = Produktivstand (148 MB, ~250–280 MB installiert), E3 = Massendaten beim Erststart nachladen, E4 = vollständiger Download beim Erststart | **E1 für iU10** (per `-p:SeedDb` gesetzt), **E2 für TestFlight** — vorher `VACUUM` der Produktiv-DB messen. E3/E4 nur bei Store-Vorgaben; On-Demand-Resources sind mit .NET für iOS nicht baubar | vor iU13 | **entschieden: E1 für die CI, E2 für TestFlight.** Die `VACUUM INTO`-Messung der Produktivdatenbank macht der Anwender unter Windows (Befehl in `Umsetzung_iU10_Nachweise.md`, Abschnitt iU13) — die Produktiv-DB liegt nicht im Repo; E3/E4 nur bei Store-Vorgaben | E1 CI, E2 TestFlight | 03.09.2026 |
| **iF26** | **KI-Semantiksuche auf iOS**: die 46-MB-`onnxruntime.xcframework` statisch mitlinken — oder die Semantiksuche auf iOS abschalten? | in iU10/iU11 **abschalten** — `ExcludeAssets="native;build;buildTransitive"` in `EPOS.iOS.csproj`. `SemantikModell` scheitert damit erst beim Aufruf, nicht beim Start; Chat und Wiki laufen über REST und sind unberührt. Entscheid vor iU13 | vor iU13 | **umgesetzt und bestätigt** (iU10-3): abgeschaltet bis iU13; ob die 46-MB-Laufzeit für das Gerät mitgelinkt wird, wird mit iU13 entschieden | bestätigt: abschalten bis iU13 | 03.09.2026 |
| **iF27** | **`bundle_green` ist tot** — fährt iOS dieselbe SQLite 3.53.3 wie Windows (`bundle_e_sqlite3`, statisch gelinkt)? | **ja.** Die Fassung 2.1.12 von `bundle_green` existiert nicht (letzte 2.1.11, in 3.0 entfallen), und die System-SQLite des Geräts wäre für die **114 STRICT-Tabellen** nicht steuerbar. Auf iOS liefert `bundle_e_sqlite3` `provider.internal` und eine statisch gelinkte `e_sqlite3.a` — dieselbe Fassung auf allen vier Läufern | iU10 | **umgesetzt und bestätigt** — CI-Lauf 33748736894 (03.09.2026): `SQLite 3.53.3`, `STRICT=114`, Projekt 1030 byte-gleich zur Windows-Basis | bestätigt | 03.09.2026 |
| **iF28** | **Mindest-iPadOS 17.0** akzeptabel? | **ja** — von `SkiaSharp.NativeAssets.iOS` 3.119 (`net8.0-ios17.0`) erzwungen; MAUI 10 selbst käme mit 15.0 aus. iPadOS 17 läuft auf iPads ab 2018 | iU10 | **umgesetzt und bestätigt** (iU10-3) | bestätigt | 03.09.2026 |
...
GDI+-Seite gibt es nur dort); hier wurde er ausschließlich übersetzt. Er war die Vorbedingung
von **iF23**.

**Nachtrag 03.09.2026.** Der Anwender hat mit „führe aus iF23 bis iF25“ die Löschung angeordnet,
ohne den Modus je unter Windows gefahren zu haben. `ChartRendererGdi.cs`,
`Referenzlauf/Bildvergleich.cs` und der Zweig `bildvergleich` in `Referenzlauf/Program.cs` sind
seither weg; ein Pixelvergleich alt/neu ist nicht mehr möglich. Der Nachweis der Bildgleichheit
besteht seither aus den drei Renderer-Tests im Kern, den `ChartProben` (Maße, Farben,
Determinismus, zehn Bilder) und dem Sichtvergleich der Berichte am Gerät.
...
P0–P5 aus [`Konzept_Umstellung_64Bit_EPOS-Plan.md`](../ueberholt/Konzept_Umstellung_64Bit_EPOS-Plan.md) sind
