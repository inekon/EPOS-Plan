...
namespace WindowsFormsApplication1
{
    ...
    internal static class WaermebedarfExternHuelle
    {
        ...
        {
            return new BlazorAssistentSeite<WaermebedarfExternDialog, Z_ProjWaermebedarfModel>(
                (projektId, projektName, modelle) => new Dictionary<string, object>(
                    Gaben(null, projektId, projektName, modelle, wizard: true)),
                MASS);
        }
            ...
            List<Z_ProjWaermebedarfModel> modelle, bool wizard)
        {
            ...
            return new Dictionary<string, object>
            {
                ...
                ["Katalog"] = new Func<IReadOnlyList<string>>(Katalognamen),
        ...
        private static IReadOnlyList<string> Katalognamen()
        {
            var ctrl = new WaermebedarfStammCtrl();
            ctrl.ReadAll();

            var namen = new List<string>(ctrl.rows);
            for (int i = 0; i < ctrl.rows; i++) namen.Add(ctrl.items[i].m_szBezeichner);
            return namen;
        }
