...
- **Die Namensregel und der Weg zur Maske.** Tolerant gelesen wird nur der **Wortlaut des Modells**, und zwar nach
  EINER Regel (`KiKern/KiWahl.cs`, KI‑D‑Q6): Schlüssel, gefalteter Schlüssel (Groß/klein, Umlaut, Unterstrich,
  Leerraum), Anzeigetext, eindeutiger Anfang in beide Richtungen, eindeutig enthaltener Teil; die erste Stufe, die
  etwas findet, entscheidet, und mehrdeutig bleibt mehrdeutig (die Absage nennt die Kandidaten). Ein Kandidat, mit
  dem der genannte Text nur BEGINNT oder den er nur ENTHÄLT, braucht mindestens drei Zeichen — ein Buchstabe ist kein
  Name. Der Treffer nennt seine Stufe (`KiWahltreffer.Stufe`). So liest die offene Maske einen Feldnamen
  (`KiMaskenbruecke.Feldsuche`), und so wird ohne offene Maske die GEMEINTE Maske aus den Feldnamen erschlossen:
  **Über alle Masken entscheidet die beste Stufe** (`KiAktionenDialog.GemeinteMaske`, #472) — exakter Schlüssel vor
  gleichem Namen (gefalteter Schlüssel und Anzeigename stehen hier gleich) vor Wortanfang vor enthaltenem Teil. Es
  zählen nur die Masken, die den Namen auf dieser Stufe treffen, ein Feld oder mehrere; eine Maske, die ihn erst
  schwächer trifft, zählt nicht mehr mit („With PV surplus" meint die Wärmesenke, nicht die Ansicht „Simulation", die
  „PV surplus" enthält). Treffen mehrere Masken auf der besten Stufe und führen sie nicht an einen Ort (Katalogeditor
  und Verwaltung), wird **benannt abgesagt**: Die Absage nennt genau diese Masken mit Anzeige- und Maskennamen
  (`KI_DLG_MASKE_MEHRDEUTIG`), nicht die Liste aller. Gehalten vom Wächter
  `KiMaskenwegTests.Kein_Anzeigename_fuehrt_in_eine_fremde_Maske` über alle Anzeigenamen in de-DE und en-US.
  **Zwischen zwei Schlüsseln gibt es keine Nachsicht:** Führt die
  gemeinte Maske auf eine andere Katalogmaske (Katalogeditor → Verwaltung, Photovoltaik des Projekts →
  Modulkatalog, Pufferverwaltung → Ansicht „Simulation", Vorlagenposition → Kostenverwaltung), nennt die Absage diese
  nur, wenn sie das Feld unter **demselben Schlüssel** oder unter dem **erklärten Gegenstück** führt
  (`KiDialoge.Zielfeldname`): der Tabelle der Katalogeditoren (`bereitschaftsverlust` → `bbverlust`) oder der Vorsilbe
  `katalog_` der Felder aus „Alle Daten" (`katalog_breite` → `breite`) — sonst nennt sie die gemeinte Maske. Ein
  ähnlicher Name ist kein Gegenstück: `wr_wirkungsgrad` (Wechselrichter) ist nicht der `wirkungsgrad` des Moduls,
  `ladeleistung` (Puffer) nicht die `speicher_ladeleistung` der Batterie, `ersatz_fuehren` nicht der `satz` einer
  Kostenzeile. Entschieden wird über Schlüssel, nie über Beschriftungen — die Antwort ist in jeder Anzeigesprache
  dieselbe. Wächter: `KiMaskenwegTests` (für jeden Feldschlüssel des Katalogs führt die genannte Maske das Feld oder
  sein erklärtes Gegenstück), `KiKatalogKulturTests` (gleiche Antwort in `de-DE`, `en-US` und invariant).
...
- **Stand der Abdeckung.** Angemeldet sind alle Masken mit Einstellwerten; die Ausnahmeliste führt keinen
  ...
  Projektdialog und führt neun Felder — Filter und Detailblock —, kein Feld für die Betriebsart „Verwaltung").
  Draußen bleibt
...
| **KI‑D‑Q4** Darf der Assistent speichern oder nur Felder füllen? | Beides, Speichern nur mit Bestätigung und Sicherungspunkt (datenbankwirksam, Aufgabensteuerung 4.4). | **entschieden 11.09.2026 (Empfehlung), umgesetzt #201**: `dialog_speichern` ruft den Speicherweg der offenen Maske — Stufe 2, `datenbankwirksam`, damit mit Sicherungspunkt VOR der Bestätigung; der Pfad steht in der Bestätigung und im Ergebnis. Ohne Freigabe wird der Speicherweg nicht einmal gerufen. **Keinen Speicherweg** haben Masken, deren einziger Schreibweg zugleich schließt oder einen Satz anlegt — dort klickt der Anwender OK, und `dialog_speichern` lehnt benannt ab (`KI_AKTION_SPEICHERN_KEIN_WEG`; Befund #472): Energieträger-Variante (OK legt einen neuen Satz an, ein „Neu…"), Kostenfaktor-Katalog („Neu" und „Löschen" schreiben sofort, OK schließt nur), BHKW-Wirtschaftlichkeit, PV-Vergütung, Tarifstruktur und Wirtschaftlichkeitsparameter (OK bzw. „Übernehmen" schreibt und schließt in einem, wie bei den Überlagerungen) |
