...
| **W15c‑O‑1** | **Der Vertragsendpunkt** (E‑17, B27). `LizenzTextCtrl.ONLINE_QUELLE` ist eine Zeile; die Umstellung auf `epos/v1/vertrag` wartet auf Server 1.4.0 und die Anwenderentscheidung. |
| **W15c‑O‑2** | **Ein `LizenzTexte`-Bündel.** Die zwei großen Komponenten tragen ihre Anzeigetexte als 25 bzw. 20 Einzelparameter. Ein Bündel wie `KiChatTexte` (W15b) würde beide unter 350 Zeilen bringen; das ist eine Aufräumarbeit, keine Fachänderung. |
