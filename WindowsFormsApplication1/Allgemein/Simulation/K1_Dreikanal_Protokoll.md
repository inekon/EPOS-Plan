...
| # | Punkt | Ziel |
|---|---|---|
| K1-O1 | `Tab_Stromverbraucher` bekommt weiterhin **keinen** `ID_Projekt`-Filter (in `ProfilQuelle.Strom` kommentiert) — die Namens-Mehrdeutigkeit ist real (`EFH_3_Pers` steht 11× in der Projekttabelle); `KopfLesen` nimmt die erste Trefferzeile. **Korrektur 27.08. (Fehlerrunde):** Die K1-Behauptung „es gibt keine `_STAMM`-Fassung" war falsch — `Tab_Stromverbraucher_STAMM`/`Tab_Stromverbrauchertyp_STAMM` existieren (41/40 Einträge); die Katalogvorschau las deshalb fälschlich die Projektkopien und zeigte für 36 von 41 Katalogeinträgen stumm Nullmonate. Behoben in der Fehlerrunde (Modus-Weiche auf `_STAMM`) | Projektfilter: Einzelfix offen |
