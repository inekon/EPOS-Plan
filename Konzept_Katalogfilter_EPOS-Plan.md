...
| **W14a‑E‑10‑O‑14** | „toten Code entfernen — nur wenn sinnvoll" (Aufgabe #153): `KatalogImportAblauf.Anzeigeindex(double von, double bis, string suchtext)` — die letzte lebende Fassung der alten Filtermechanik im Kern-Ablauf, seit S3.4 ohne Aufrufer im Produktcode (siehe O‑12) — wird samt ihrem einzigen Prüffall entfernt | 09.09.2026 | **Entfernt.** `KatalogImportAblauf.cs`: Methode, XML-Kommentar und Abschnittsüberschrift „2 — Filtern" gestrichen, Folgeabschnitte umnummeriert (3 → 2 Vorprüfen, 4 → 3 Ausführen); Klassenkommentar nennt seither nur noch „Lesen, Vorprüfen, Ausführen" mit einem Verweis auf `Katalogfilter`. `KatalogImportAblaufTests.cs`: Fall `DerFilterVerbindetZahlenbereichUndSuchtext` samt Abschnittskopf „3 — Filtern" gestrichen (Folgeabschnitte 4 → 3, 5 → 4); die Probe `pufferspeicher_vaillant.vdi` bleibt (neun weitere Fälle nutzen sie). Geprüft und GEBLIEBEN, weil beide weitere lebende Aufrufer haben: `KatalogImportSatz.Filterwert` (u. a. `EPOS.UI/Dialoge/Import/KatalogImportDaten.cs`, `WindowsFormsApplication1/Views/Import/KatalogImportHuelle.cs`) und `VdiAuswahlFilter.Passt(string, params string[])` (u. a. `Katalogfilter.cs` zweimal, `EnergietraegerDialog.razor`). Suche und Zahlenausdruck bleiben belegt durch `ZahlenausdruckTests.Bereich`, `KatalogspaltenfilterTests.Suche_ist_ODER_ueber_Spalten_und_UND_ueber_Begriffe` und den Wirt-Test `KatalogImportDialogTests.Der_Zahlenfilter_und_der_Suchtext_wirken_zusammen`. `EPOS.Kern.Tests` 2195 → 2194, `EPOS.UI.Tests` unverändert 3362, Build 0 Fehler/19 Warnungen (unverändert), Referenzlauf 1030 gegen `2026-09-07_R6_PvKoeffizienten` byte-gleich (22 Dateien, 236680 Werte) — der Importpfad rechnet nicht mit. Commit mit Präfix `W14a-E-10-O-14:` |
...
| **O‑12** | ~~**`ModulImportProfil` und `KatalogImportProfil` führen Daten, die keine Maske mehr liest**: `FilterHersteller`, `FilterTechnologie`, `FilterSuche`, `SuchePlatzhalter`, `TextAlle`, `Zahlenfilter` samt Vorbelegungen und `KatalogImportProfil.FilterVon`/`FilterBis`/`FilterMaximum`/`FilterBezeichnung`/`Zweitfilter`/`HerstellerFilter`.~~ **Erledigt am 07.09.2026** (`7c196c5`). **Die Aufzählung war eine Verdachtsliste, keine Streichliste** — gemessen wurde jedes Feld einzeln über `*.cs` **und** `*.razor` (Messtabelle im Protokoll `iU9_W15a_Blazor_Port_Protokoll.md`, Abschnitt „W14a‑E‑10‑O‑12“). **Gefallen sind zwölf Namen und ein Hilfstyp:** aus `KatalogImportProfil` `FilterBezeichnung`, `FilterVon`, `FilterBis`, `FilterMaximum` und `HerstellerFilter` samt ihren Setzern in allen fünf Ausprägungen; aus `KatalogFilterbereich` die vier Teile `Bezeichnung`, `Von`, `Bis`, `Maximum`; aus `ModulImportProfil` `FilterHersteller`, `FilterTechnologie`, `FilterSuche`, `SuchePlatzhalter`, `TextAlle` und die Eigenschaft `Zahlenfilter` samt dem Hilfstyp `ImportZahlenfilter`, der danach keinen Halter mehr hatte; dazu `ImportZeile.Hersteller`/`.Technologie` (die zwei Größen der gefallenen Klapplisten — dieselben Werte stehen als Spalten `HERSTELLER`/`TECHNOLOGIE` in der Zeile) und die fünf `IMP_KAT_FILTER_*`-Fälle in `EPOS.UI.Dialoge.Import.Texte.Zu`, deren einziger Aufrufer `FilterBezeichnung` war. **Stehen geblieben ist, was einen Leser hat** — und das ist die interessantere Hälfte: **`Zweitfilter` lebt** (`KatalogImportDialog.razor:629` liest `Zweitfilter?.Nachkommastellen` für die zweite Zahlenspalte des Stromspeicherimports, W13‑E‑2), ebenso `FilterNachkommastellen` (dieselbe Stelle), `FilterSpaltentitel` und `FilterSpalteneinheit` (sie bauen in `BaueListenprofil` den Kopf der Zahlenspalte — ein Feld, das nur noch einen Spaltentitel erzeugt, ist **nicht** tot), `Filterspalte`, `Zweitfilterspalte`, `Zahlspalten`, `Quellen`, `Listenspalten` und `Hinweis`. **Nachtrag 09.09.2026 (O‑14):** Damals bewusst NICHT angefasst blieb `KatalogImportAblauf.Anzeigeindex(von, bis, suchtext)` — sie hatte zwar keinen Aufrufer im Produktcode mehr, aber ihr einziger Prüffall war der einzige Ort, an dem die Regel „Zahlenbereich UND Suchtext" gegen eine echte VDI-Datei (9 Sätze, 7 unter 1 000 l, 6 „exclusiv") zugesichert wurde — die Katalogliste erbringt dasselbe, aber ihr Zeilenaufbau liegt im Razor-Dialog und nicht im Kern. Mit dem Anwenderentscheid „toten Code entfernen — nur wenn sinnvoll" (Aufgabe #153) ist auch dieser letzte Nutzer gefallen: Methode und Prüffall sind entfernt, Suche und Zahlenausdruck bleiben durch `ZahlenausdruckTests`, `KatalogspaltenfilterTests` und `KatalogImportDialogTests` belegt — siehe **O‑14**. **Keine Zusicherung ging verloren:** Die vier Fälle der Filtervorbelegung prüfen weiter Nachkommastellen und Dateifilter (die Vorbelegung selbst gibt es seit O‑10 nicht mehr), `NotEmpty(Zahlenfilter)` ist zu `NotEmpty(Zahlspalten)` geworden, und die Spaltenköpfe deckt `KatalogfilterImportTests` seit S3.4 ab. Kein Verhalten ändert sich — 2 033 / 3 239 / 337 / 469 grün in `de` und `en_US`, 44 Bilder grün, 0 SQL-Fundstellen |
| **O‑14** | ~~**`KatalogImportAblauf.Anzeigeindex(double von, double bis, string suchtext)` hat seit S3.4 keinen Aufrufer im Produktcode mehr** (siehe O‑12) — nur ihr eigener Prüffall `DerFilterVerbindetZahlenbereichUndSuchtext` sicherte sie noch zu.~~ **Entfernt am 09.09.2026** (Anwenderentscheid „toten Code entfernen — nur wenn sinnvoll", Aufgabe #153, Kennung `W14a‑E‑10‑O‑14`, Zeile in Kapitel 9). Gemessen mit `grep -rn "Anzeigeindex(" --include=*.cs --include=*.razor` (ohne `bin`/`obj`): außerhalb des einen Prüffalls kein Aufrufer; die gleichnamigen PRIVATEN Helfer `Anzeigeindex(Katalogfilterzeile)` in `KatalogImportDialog.razor` und `ModulImportDialog.razor` sind eine andere Sache (bilden `Katalogfilterzeile` → Listenindex ab, nicht Satz → Zahlen-/Suchfilter) und bleiben unberührt. Mit ihr NICHT tot geworden: `KatalogImportSatz.Filterwert` (weitere Aufrufer u. a. in `EPOS.UI/Dialoge/Import/KatalogImportDaten.cs` und `WindowsFormsApplication1/Views/Import/KatalogImportHuelle.cs`) und `VdiAuswahlFilter.Passt(string, params string[])` — es gibt keine zweite Überladung `Passt(string, string, string)`, der Aufruf mit zwei Namen band nur die `params`-Form, die `Katalogfilter.cs` selbst zweimal ruft. Suche und Zahlenausdruck bleiben belegt durch `ZahlenausdruckTests`, `KatalogspaltenfilterTests` und `KatalogImportDialogTests` (Kapitel 9, Zeile `W14a‑E‑10‑O‑14`) |
