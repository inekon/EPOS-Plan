...
- **„Einmal täglich" ist wörtlich umgesetzt** (Anwenderentscheid iF30‑O‑2 vom 06.09.2026, „einmal täglich reicht"): Die drei Warnstufen erscheinen **einmal je Kalendertag** statt bei jedem Programmstart. `LizenzWarnungMerker` hält dazu in `Dienste.Einstellungen` (`LizenzWarnungGezeigt`) einen Vermerk `yyyy-MM-dd|stufe`; eine dringendere Stufe (30 → 14 → 7) zeigt auch am selben Tag noch einmal, ein unlesbarer Vermerk zeigt immer. Entschieden wird das im Kern (`LizenzLage.MitTagesmerker` → `LizenzLage.WarnungZeigen`), die Wurzel fragt nur. **Der Lesemodus ist keine Warnstufe** und bleibt — wie Kulanzfenster und fällige Nachprüfung — bei jedem Start sichtbar.
