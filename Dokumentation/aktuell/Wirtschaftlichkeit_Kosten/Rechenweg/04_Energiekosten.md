...
109): Sie steht beim Wiederöffnen wieder da, auch wenn der Brennstoff keine Regel nach kWh führt; leer heißt
Abrechnungseinheit — so beginnen neue Zuordnungen (E7c2‑Q3). Führt eine Datenbank vor Schritt 109 die Spalte
nicht, zeigt die Karte die Abrechnungseinheit und nennt den Grund. Die Gase des Brennstoffkatalogs führen
ihre Menge in Nm³ (€/Nm³), der Brennstoff „Sonstige" in kWh (€/kWh) — Schemaschritt 110, reiner Stammtext;
die Rechnung unten bleibt davon unberührt.
...
Arbeitspreis
  ...
  Die Arbeit dieser Kette — Menge × Arbeitspreis, ohne Grund- und Leistungspreis — ist auch die
  Bezugsgröße der Betriebskostenarten „% der Brennstoffkosten" (Σ aller Brennstoffmodule) und
  „% der Stromkosten" (Netzbezug × Arbeitspreis des Stromträgers), frisch aus dem jüngsten Lauf
  (B-4, E7c2-Q2; Rechenweg 02)
...
| ✔ U32 | Die Preisbasis ging als Regelkennung `ID_Umrechnung` mit; ohne Regel nach kWh wurde −1 abgelegt, und die Wahl kWh fiel beim nächsten Öffnen auf die Abrechnungseinheit zurück (Rest von ET-D-3) | **umgesetzt #446** (E7c2, Schemaschritt 109): eigene Spalte `energy_project_settings.Preisbasis`, einmalig aus `ID_Umrechnung` befüllt (Testdatenbank: 5 Zeilen „kWh", 23 mit der Abrechnungseinheit); Preise und Kosten unverändert — Proben 1030 (Erdgas E mit Regel nach kWh) und 1024 (Stadtgas ohne Regel) mit Basis kWh gleich |
| ✔ U-1 | Einheitenbruch: `Tab_Brennstoff_Stamm.Einheit` der fünf Gase „m³" gegen `energy_carrier.billing_unit` „Nm³" — die Identitätsregel-Ableitung lieferte −1 | **umgesetzt #446** (E7c2, Schemaschritt 110, reines DML; A9): Einheit und Preiseinheit der fünf Gase auf Nm³, dazu eine Preiszeile (Projekt 1039); der Brennstoff 24 „Sonstige" auf kWh (E7c2‑Q4, Anwender) — ohne Heizwert (H_i = H_s = 0) nur als Einheitentext; Energiekosten und Emissionen der Basisprojekte unverändert, der Vorlagenbau läuft ohne Auffälligkeit. Das Beispiel dieses Papiers schreibt die Gasmenge weiter in m³ |
