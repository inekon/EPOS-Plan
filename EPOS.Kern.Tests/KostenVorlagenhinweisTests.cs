...
namespace EPOS.Kern.Tests
{
    ...
    /// 1030 rechnet „Hilfsenergiekosten (Strom)" als „% der Endenergiekosten", während
    /// die Standardvorlage seit Schemaschritt 94 „% des Endenergiebedarfs" führt.</para>
    /// </summary>
    ...
    public class KostenVorlagenhinweisTests : IDisposable
    {
        ...
        /// <summary>Die Zeile, deren Bemessung von der Vorlage abweicht.</summary>
        ...
        public void Nur_eine_abweichende_Bemessung_bekommt_den_Hinweis()
        {
            ...
            KostenHerleitung.Angabe hilfsstrom = Bilde(zeilen[POS_HILFSSTROM]);
            Assert.Equal(DbWerte.BEMESSUNG_PROZENT_ENDENERGIEBEDARF, hilfsstrom.VorlagenBemessung);
            Assert.Contains("Standard", hilfsstrom.VorlagenZeile, StringComparison.Ordinal);
            Assert.Contains(BemessungKatalog.Anzeige(DbWerte.BEMESSUNG_PROZENT_ENDENERGIEBEDARF,
                                                     KOMPONENTE_KESSEL),
                            hilfsstrom.VorlagenZeile, StringComparison.Ordinal);

            Assert.Equal("", Bilde(zeilen[POS_WARTUNG]).VorlagenZeile);
        ...
        private static Dictionary<string, KostenProjektPositionenCtrl.Zeile> Zeilen()
        {
            ...
            Assert.True(karte.ContainsKey(POS_HILFSSTROM),
                        "Die Hilfsstromzeile des Kessels fehlt in der Testdatenbank.");
            return karte;
        }
