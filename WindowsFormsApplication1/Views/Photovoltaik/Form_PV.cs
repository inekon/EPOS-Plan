...
namespace WindowsFormsApplication1
{
    public partial class Form_PV : Form
    {
        ...
        private PhotovoltaikCtrl pvctrl = new PhotovoltaikCtrl();
        ...
        private void Form_PV_Load(object sender, EventArgs e)
        {
            ...
            rs.Open("SELECT Firma FROM Tab_PV GROUP BY Firma ORDER BY Firma");
            while (rs.Next())
            {
        ...
        private void btn_Hinzu_Click(object sender, EventArgs e)
        {
            ...
            rs.Open("select * from Tab_PV where Modulname='" + listBox_DB.Text + "'");
            if (rs.Next())
            {
        ...
        private void listBox_Auswahl_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            rs.Open("select * from Tab_PV where Modulname='" + listBox_Auswahl.Text + "'");
            if (!rs.EOF())
            {
                textBox_Name.Text = (string)rs.Read("Modulname");
        ...
        private void SetFilter()
        {
            ...
            if (comboBox_Hersteller.Text == "Alle") szFilter = "Modulname Like '%'";
            else szFilter = "Firma='" + comboBox_Hersteller.Text + "'"; 

            listBox_DB.Items.Clear();
            sql = "select * from Tab_PV where " + szFilter;
            rs.Open(sql);

            while (rs.Next())
            {
                listBox_DB.Items.Add((string)rs.Read("Modulname"));
            }
            rs.Close();
        }
        ...
        private void btn_Löschen_Click(object sender, EventArgs e)
        {
            ...
            {
                RecordSet rs = new RecordSet();
                rs.Open("Delete * from Tab_PV where Modulname='" + listBox_DB.Text + "'");
                rs.Close();
