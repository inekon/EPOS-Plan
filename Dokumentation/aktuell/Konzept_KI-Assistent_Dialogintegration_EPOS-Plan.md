...
- **Stand der Abdeckung.** Angemeldet sind alle Masken mit Einstellwerten bis auf drei: Zapfprofilgenerator,
  Zapfprofilauslegung und Bedarfstag-Konstruktor stehen als `Offen` mit dem Auftrag „#458 Stufe 3 nach Z3" (sie werden
  in der Zapfprofil-Sitzung umgebaut). Draußen mit Vermerk in der Eingabebilanz bleibt daneben, mit Stufe 3, die
  Rechenweg-Wahl der Bedarfsprofile; die Zahlenfolgen (Monats-, Stunden- und Wochenwerte) sind seit Stufe 3b
  Zahlenreihen (nächster Punkt). **Führt eine Maske Felder als Daten eines Profils** (die Verwaltungen, „Alle Daten" der
