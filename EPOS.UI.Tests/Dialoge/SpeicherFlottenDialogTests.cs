...
public sealed class SpeicherFlottenDialogTests : EposBunitContext
{
    ...
    public void Echter_Einstieg_zeigt_Flotteneditor_statt_altem_Einzelspeicherraster()
    {
        ...
        Assert.Contains(Resource.FLOTTE_DLG_REITER_FLOTTE.Replace("&", "&amp;"), cut.Markup);
    ...
    public void Berechnung_bekommt_alle_physischen_Einheiten_als_unabhaengigen_Stand()
    {
        ...
        cut.FindAll("button").Single(b => b.TextContent.Trim() == Resource.FLOTTE_DLG_BTN_RECHNEN).Click();
    ...
    public async Task Projektuebernahme_ist_explizit_und_nach_Eingabeaenderung_gesperrt()
    {
        ...
        cut.FindAll("button").Single(b => b.TextContent.Trim() == Resource.FLOTTE_DLG_REITER_FLOTTE).Click();
        await cut.InvokeAsync(() => cut.FindComponent<SpeicherFlottenEditor>().Instance.WertChanged.InvokeAsync(config));
        cut.FindAll("button").Single(b => b.TextContent.Trim() == Resource.FLOTTE_DLG_REITER_ERGEBNIS).Click();
    ...
    /// ändern — der Dialog wechselt darauf auf seinen ERSTEN Reiter, wo der Editor steht.
    /// </summary>
    [Fact]
    public async Task Aus_dem_Ergebnis_fuehrt_ein_Weg_zurueck_zur_Betriebsfuehrung()
    {
        var config = new FlottenStudieKonfiguration
        {
            Einheiten = new() { new() { Id = "a", Name = "A", KapazitaetKWh = 20, LadeleistungKw = 5,
                EntladeleistungKw = 5, Ladewirkungsgrad = 0.95, Entladewirkungsgrad = 0.95,
                SocMin = 0.1, SocMax = 0.9, SocStart = 0.5 } },
            Optionen = new() { EnergieAusgleichEuroProKWh = 0.2, Betriebsziel = FlottenBetriebsziel.PeakShaving, WirtschaftlicherPeakZielwertKw = 8 },
            Wirtschaftlichkeit = new() { ProjektjahreBeiWiederholung = 1 }
        };
        var input = new FlottenEingang { DatenId = "Test", Istwerte = new()
        {
        ...
        // Im Ergebnisreiter steht KEIN Betriebseditor mehr - nur der Weg dorthin.
        ...
        cut.WaitForAssertion(() => Assert.Single(cut.FindComponents<SpeicherFlottenBetriebEditor>()));
    }
}
