...
| **A‑10** | **Der Flackerschutz der Semantikzeile entfällt.** | Der Bestand schrieb sie nur bei ZUSTANDSWECHSEL, weil „jedes Setzen von `Text` das Label neu zeichnet, und das sähe man". Blazor zeichnet nur, was sich ändert; ein Merker wäre eine zweite Wahrheit. Der Tooltip mit Modell/Lizenz/Herkunft entfiel mit ihm — er stand an einem Label, das es nicht mehr gibt (offener Punkt W15b‑O‑2). **Nachgetragen am 04.09.2026** (Entscheid W15b‑O‑2): Er hängt jetzt als `title` an `.epos-kichat-status` und erscheint nur, wenn die Semantikzeile etwas sagt — der Flackerschutz bleibt trotzdem weg, er betraf das Schreiben des Zeilentextes, nicht den Tooltip. |
...
| **W15b‑O‑1** | Die Naht `IKiAusfuehrung` (§ 6, B31) — ist die Schnittstelle so richtig geschnitten? **Und was heißt sie für iOS?** | **Bestätigt 04.09.2026** (Empfehlung angenommen). Sie hat sieben Glieder und wird nur von `KiChatService` gerufen. Ein engerer Schnitt ginge nicht, ohne den Dienst zu ändern; ein weiterer wäre ein zweiter Weg an der Schutzkette vorbei. **Zur iOS-Frage:** `EPOS.iOS` referenziert denselben `EPOS.Kern` unverändert — solange die Hülle dort keine eigene `IKiAusfuehrung`-Fassung einlegt, gilt die stille Standardfassung `KeineAusfuehrung`: leeres Register, **jede Aktion abgelehnt**. Der Chat kann auf dem Gerät also fragen und suchen, aber nichts ausführen; die iOS-Fassung ist W15b‑O‑4/iU11. |
| **W15b‑O‑2** | Der Tooltip der Semantikzeile (Modell / Lizenz / Herkunft) fehlt. | **Nachgetragen 04.09.2026** (Empfehlung angenommen): `title` an `.epos-kichat-status`, gefüllt aus `KI_SEMANTIK_HERKUNFT` — dem Schlüssel des Vorläufers, **wortgleich**, in beiden Sprachen schon vorhanden — mit `SemantikModell.NAME`/`LIZENZ`/`QUELLE`. Den Text baut die **Hülle** (`KiChatHuelle.Texte()`, wie `VorschauKopf`) und reicht ihn als `KiChatTexte.SemantikHerkunft` herein; die Komponente kennt weder `SemantikModell` noch einen Ressourcenschlüssel. Er erscheint **nur, wenn die Semantikzeile Text hat** — steht dort allein der Tagesverbrauch, gibt es gar kein `title`. Zwei Zeugen in `KiChatDialogTests`. |
...
| Nr | Punkt |
|---|---|
| ~~**W15b‑O‑1**~~ | Die Naht `IKiAusfuehrung` (B31) ist eine Entscheidung dieser Welle, keine der Vermessung — sie sollte bestätigt werden. → **bestätigt 04.09.2026 (Empfehlung angenommen)**. Der Schnitt bleibt, wie er ist. Zur Frage des Anwenders, was das für iOS heißt: **Die iOS-Hülle nutzt denselben Kern** — `EPOS.iOS` referenziert `EPOS.Kern` unverändert —, und solange sie keine eigene `IKiAusfuehrung`-Fassung einlegt, gilt dort die stille Standardfassung `KeineAusfuehrung`: jede Aktion wird abgelehnt, der Chat kann fragen und suchen, aber nichts ausführen; die iOS-Fassung ist W15b‑O‑4/iU11. |
| ~~**W15b‑O‑2**~~ | Der Tooltip der Semantikzeile (Modell / Lizenz / Herkunft) ist mit dem Label entfallen (A‑10). Nachtragbar als `title` an `.epos-kichat-status`. → **nachgetragen 04.09.2026 (Empfehlung angenommen)**, siehe § 5. |
