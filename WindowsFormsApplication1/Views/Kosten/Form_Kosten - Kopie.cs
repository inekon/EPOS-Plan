...
namespace WindowsFormsApplication1
{
    public partial class Form_Kosten : Form
    {
        ...
        public Form_Kosten(int IDProjekt)
        {
            ...
            LoadDatenAusDictionary(m_ID_Projekt);
        ...
        // DATEN LADEN
        public void LoadDatenAusDictionary(int projektID)
        {
        ...
        public bool SaveDatenAusDictionary(int projektID)
        {
            ...
            { 
                using (OleDbConnection conn = new OleDbConnection(connString))
                {
                    ...
                    {
                        string bez = entry.Key;      // z.B. "BHKW_Nutzungsdauer"
                        decimal wert = entry.Value.Value; // Der aktuelle Wert aus dem NumericUpDown
                        double wertFuerDB = Convert.ToDouble(entry.Value.Value);

                        // SQL Update/Insert Logik (wie zuvor, aber basierend auf dem Key)
                        ...
                        using (OleDbCommand cmd = new OleDbCommand(sql, conn))
                        {
                            ...
                            {
                                // Falls noch kein Wert existiert: INSERT
                                // Das SELECT liefert 3 Spalten: 
                                // 1. @pid (für ProjektID)
                                // 2. ID (aus Tab_KostenStamm für StammID)
                                // 3. @wert (für EingegebenerWert)

                                // Wir mappen: ProjektID -> @pid, StammID -> StammID (aus der DB), EingegebenerWert -> @wert
                                ...
                                using (OleDbCommand insCmd = new OleDbCommand(insSql, conn))
                                {
                                    // OLEDB REIHENFOLGE (von links nach rechts im SQL):

                                    // 1. @pid (erscheint zuerst im SELECT)
                                    insCmd.Parameters.AddWithValue("@pid", projektID);

                                    // 2. @wert (erscheint als zweites hinter 'StammID')
                                    insCmd.Parameters.AddWithValue("@wert", wertFuerDB);

                                    // 3. @bez (erscheint ganz hinten im WHERE)
            ...
            {
                // Spezifische Datenbankfehler (Pfad falsch, Access-Treiber fehlt)
                //MessageBox.Show("Datenbankfehler: " + exDb.Message, "Fehler", MessageBoxButtons.OK, MessageBoxIcon.Error);
                return false;
            }
            catch (Exception ex)
            {
                // Allgemeiner Fehler (Konvertierung etc.)
