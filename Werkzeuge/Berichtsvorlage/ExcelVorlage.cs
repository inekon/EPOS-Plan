...
namespace Berichtsvorlage
{
    ...
    /// neu — Beispiel- und Standardvorlage, Kurzbericht, ausführliche Vorlage und Bausteinvorlage (<c>.dotx</c>) je Sprache, die ausführliche
    /// Excel-Vorlage je Sprache —, nachdem die Stilvorlage <c>Berichtsvorlage.docx</c> bereinigt ist (ein zweiter Lauf findet nichts). Jeder
    ...
    internal static class Sammellauf
    {
        ...
        /// <summary>
        /// Die Schritte in ihrer Folge. <b>Erweiterungsstelle:</b> Weitere mitgelieferte Vorlagen kommen hier als eigener Schritt
        /// hinzu — dann erzeugt sie auch <c>alle</c>, und die Aktualitätswache
        /// hält sie.
        /// </summary>
        internal static IReadOnlyList<Sammelschritt> Schritte()
        {
            ...
            return new List<Sammelschritt>
            {
                ...
                // Die Bausteinvorlage je Sprache entsteht aus der Standardvorlage (Schritt oben), nicht aus der Stilvorlage.
                new Sammelschritt(BerichtsvorlagenCtrl.DATEI_BAUSTEINE, (q, z, a) => Bausteine.Ausfuehren(Standard(z), z, false, a)),
                new Sammelschritt(BerichtsvorlagenCtrl.DATEI_BAUSTEINE_EN, (q, z, a) => Bausteine.Ausfuehren(Standard(z), z, true, a)),
            };
        }

        /// <summary>Die Standardvorlage neben <paramref name="ziel"/> — die Quelle der Bausteinvorlage.</summary>
        private static string Standard(string ziel)
        {
            return Path.Combine(Path.GetDirectoryName(ziel) ?? "", BerichtsvorlagenCtrl.DATEI_STANDARD);
        }
