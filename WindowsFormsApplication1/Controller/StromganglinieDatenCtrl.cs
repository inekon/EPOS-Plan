                ...
                using (OleDbConnection conn = new OleDbConnection(DataRepository.GetConnectionString()))
                {
                    ...
                    using (OleDbTransaction trans = conn.BeginTransaction())
                    {
                        ...
                        using (OleDbCommand cmdCount = new OleDbCommand("SELECT COUNT(*) FROM Tab_Stromganglinie", conn, trans))
                        {
                            ...
                            if (count > 0)
                            {
                                using (OleDbCommand cmdMax = new OleDbCommand("SELECT MAX(ID_GanglinieDaten) FROM Tab_Stromganglinie", conn, trans))
                                {
                        ...
                        string sqlKopf = "INSERT INTO Tab_Stromganglinie (ID_GanglinieDaten, Bezeichner, Zeitinterval) VALUES (?, ?, ?)";
