...
- Testsammlungen laufen **nicht parallel** (Kulturpinnung in vielen Testklassen), deshalb die
  ...
  `EPOS.UI.Tests` tragen dieselben Werte seit #531 als `xunit.runner.json` — Läufe ohne
  Schalter sind damit reihenfest, die Schalter bleiben.
- `EPOS.Kern.Tests` und `EPOS.UI.Tests` laufen seit #531 unter der Standardkultur **en-US**
...
| `Werkzeuge/Berichtsvorlage` | bereinigt die Word-Stilvorlage des Berichts (doppelte Stile, Format „EPOS Kapitelkopf“) und baut daraus die Beispielvorlage mit Platzhaltern und mit `--standard` die ausgelieferte Standardvorlage `Berichtsvorlage_Standard.docx`, alles nur bei grünem `OpenXmlValidator`; `BerichtsvorlageDateiWacheTests` und `AuslieferungsvorlagenWacheTests` halten die drei Dateien und beide Lieferwege. **Nach jeder Änderung an einer der Vorlagen ziehen**; Einzelheiten in [`LIESMICH.md`](Werkzeuge/Berichtsvorlage/LIESMICH.md) | `dotnet run --project Werkzeuge/Berichtsvorlage -c Release -- bereinigen <docx>` / `… beispiel <quelle.docx> <ziel.docx> [--standard] [--katalogfassung <n>]` |
