...
- **Produktiv-DB** (`%USERPROFILE%\source\repos\WP-PLAN\Kenndaten.accdb`, per DSN TEST): 30 → 11 Abfragen,
...
## 13 Nachtrag 21.08.2026: Migrationsschritt 36 — `Abfrage_Energietraeger_Effektiv` (Empfehlung aus § 12 umgesetzt)
...
| | |
|---|---|
| Schritt | **36** — `SCHRITT_36_ENERGIETRAEGER_ABFRAGE` (auf dem Zweig als **30** gebaut, als 29 der höchste Stand war; bis zum Merge `483b605` vom 22.08.2026 war die 30 längst an `SCHRITT_30_KATALOG_DUBLETTEN_ALLE` vergeben und der Stand auf 35 gewachsen — deshalb umnummeriert) |
| `ZIEL_VERSION` | 35 → **36** (`SchemaMigration.cs:77`; auf dem Zweig noch 29 → 30) |
...
**Warum die Umnummerierung nicht kosmetisch ist:** Der Lauf überspringt jeden Schritt
mit `Nr <= version` als „bereits erledigt". Als 30 wäre er auf keiner Datenbank ab
Marker 30 je gelaufen — erst die 36 macht ihn wirksam.
...
**Aufbau** (`Schritt_36_EnergietraegerAbfrage`): 36a Basistabellen-Gurt
(`energy_project_settings`, `energy_carrier` — bei Schritt 36 ohnehin garantiert, ohne sie
scheitert schon Schritt 12); 36b Existenz-Probe (`SELECT TOP 1`, Muster `TabellenSchema`)
→ „bereits vorhanden — nichts zu tun"; 36c `CREATE VIEW`; 36d Probelesen (`COUNT(*)`), wie
...
3. Lauf 1: 36a beide Basistabellen lesbar, 36b „fehlt", 36c OK, 36d 22 Zeilen;
   **alle 22 Zeilen feldweise identisch** mit der Baseline (sortierter Volltextvergleich).
4. Lauf 2: 36b „vorhanden" → **0 Anweisungen ausgeführt** (Zweitlauf-0-Beleg).
