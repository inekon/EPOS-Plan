...
| `komponente_uebernehmen` | `von_projekt`, `nach_projekt`, `gewerk` | `KomponentenUebernahmeCtrl.Uebernehmen` — `Controller\KomponentenUebernahmeCtrl.cs:231` (eigene Transaktion um Löschen/Anlegen, Commit in `Uebernehmen`) | **zwingend vorher** `Planen` `:167` und Anzeige des `Klartext` | `fehler`/`hinweise` als `out`; Schritte 8/9 (Senken/Variante) laufen NACH dem Commit — Teilzustand möglich, wird gemeldet |
...
[`Projekte/Konzept_KI-Assistent_Dialogintegration_EPOS-Plan.md`](Konzept_KI-Assistent_Dialogintegration_EPOS-Plan.md)
